...
- CLI: add `special-agent system` for system events + heartbeat controls; remove standalone `wake`. (commit 71203829d) https://docs.openclaw.ai/cli/system
- CLI: add live auth probes to `special-agent models status` for per-profile verification. (commit 40181afde) https://docs.openclaw.ai/cli/models
...
- CLI: add `special-agent update wizard` for interactive channel selection and restart prompts. https://docs.openclaw.ai/cli/update
...
- ACP: add `special-agent acp` for IDE integrations. https://docs.openclaw.ai/cli/acp
- ACP: add `special-agent acp client` interactive harness for debugging. https://docs.openclaw.ai/cli/acp
...
- Exec approvals: migrate approvals to `~/.special-agent/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events. https://docs.openclaw.ai/tools/exec-approvals
- Nodes: add headless node host (`special-agent node start`) for `system.run`/`system.which`. https://docs.openclaw.ai/cli/node
...
- Plugins: add Zalo Personal plugin (`@special-agent/zalouser`) and unify channel directory for plugins. (#1032) — thanks @suminhthanh. https://docs.openclaw.ai/plugins/zalouser
...
- **BREAKING:** `special-agent hooks` is now `special-agent webhooks`; hooks live under `special-agent hooks`. https://docs.openclaw.ai/cli/webhooks
...
- macOS: prompt to install the global `special-agent` CLI when missing in local mode; install via `openclaw.ai/install-cli.sh` (no onboarding) and use external launchd/CLI instead of the embedded gateway runtime.
