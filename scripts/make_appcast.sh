...
ZIP=${1:?"Usage: $0 Special Agent-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/special-agent/special-agent/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Special Agent-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/special-agent/special-agent/releases/download/v${VERSION}/"}
