...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SPECIAL_AGENT_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${SPECIAL_AGENT_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${SPECIAL_AGENT_INSTALL_LATEST_OUT:-}"
fi
