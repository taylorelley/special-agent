...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/special-agent/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Special Agent Re-Auth" -c "After getting key, run: ssh $SERVER '~/special-agent/scripts/mobile-reauth.sh'" --id special-agent-auth
        ;;
    *)
