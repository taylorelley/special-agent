...
BASE_IMAGE="${BASE_IMAGE:-special-agent-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-special-agent-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=special-agent.sandbox=1)
NOTE
