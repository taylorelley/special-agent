...
const shouldBuild = () => {
  if (env.SPECIAL_AGENT_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message) => {
  if (env.SPECIAL_AGENT_RUNNER_LOG === "0") {
    return;
  }
  process.stderr.write(`[special-agent] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["special-agent.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
