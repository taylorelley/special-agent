#!/usr/bin/env bash
# Reset Special Agent like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${SPECIAL_AGENT_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="SpecialAgent.app/Contents/MacOS/Special Agent"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Special Agent"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Special Agent"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Special Agent"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.special-agent.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/special-agent-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${SPECIAL_AGENT_RESTART_LOG:-/tmp/special-agent-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${SPECIAL_AGENT_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.special-agent/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  SPECIAL_AGENT_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node special-agent.mjs daemon install --force --runtime node"
      log "  node special-agent.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.special-agent/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.special-agent.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Special Agent instances"
kill_all_special-agent
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Special Agent"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/SpecialAgent.app" ]]; then
    APP_BUNDLE="/Applications/SpecialAgent.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/SpecialAgent.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/SpecialAgent.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/SpecialAgent.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set SPECIAL_AGENT_APP_BUNDLE to your installed SpecialAgent.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node special-agent.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node special-agent.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".special-agent", "special-agent.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: Special Agent is running."
else
