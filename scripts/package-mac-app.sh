...
# Build and bundle Special Agent into a minimal .app we can open.
# Outputs to dist/SpecialAgent.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/SpecialAgent.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Special Agent"
BUNDLE_ID="${BUNDLE_ID:-ai.special-agent.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/special-agent/special-agent/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Special Agent/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :Special AgentBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :Special AgentGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Special Agent"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Special Agent"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Special Agent"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Special Agent" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Special Agent/Resources/SpecialAgent.icns" "$APP_ROOT/Contents/Resources/SpecialAgent.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Special Agent/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying SpecialAgentKit resources"
SPECIAL_AGENTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/SpecialAgentKit_SpecialAgentKit.bundle"
if [ -d "$SPECIAL_AGENTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/SpecialAgentKit_SpecialAgentKit.bundle"
  cp -R "$SPECIAL_AGENTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/SpecialAgentKit_SpecialAgentKit.bundle"
else
  echo "WARN: SpecialAgentKit resource bundle not found at $SPECIAL_AGENTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Special Agent"
killall -q Special Agent 2>/dev/null || true
