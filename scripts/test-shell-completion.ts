 ...
 * `special-agent update`. Use it to verify the completion installation flow
...
\`special-agent update\`, \`special-agent doctor\`, and \`special-agent onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# SpecialAgent Completion' block from your shell profile"),
    );
