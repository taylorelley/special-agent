...
# Special Agent Auth Widget for Termux
...
# Check auth status
termux-toast "Checking Special Agent auth..."

STATUS=$(ssh "$SERVER" '$HOME/special-agent/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/special-agent/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|SPECIAL_AGENT_EXPIRING|SPECIAL_AGENT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/special-agent/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|SPECIAL_AGENT_EXPIRED|SPECIAL_AGENT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Special Agent won't work until re-auth"
                ;;
        esac
        ;;
