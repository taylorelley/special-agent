#!/bin/bash
# Setup Special Agent Auth Management System
...
echo "=== Special Agent Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'special-agent-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Special Agent message: Send warning via Special Agent itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/special-agent-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/special-agent-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/special-agent-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now special-agent-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status special-agent-auth-monitor.timer"
echo ""
echo "Quick commands:"
