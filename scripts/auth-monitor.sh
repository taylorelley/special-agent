...
#
# Suggested cron: */30 * * * * /home/admin/special-agent/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Special Agent notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., special-agent-alerts)
...
STATE_FILE="$HOME/.special-agent/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Special Agent if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use special-agent
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Special Agent to $NOTIFY_PHONE..."
            special-agent send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
    ...
    if [ -n "$NOTIFY_NTFY" ]; then
        ...
        curl -s -o /dev/null \
            -H "Title: Special Agent Auth Alert" \
            -H "Priority: $priority" \
            -H "Tags: warning,key" \
            -d "$message" \
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Special Agent is down. Run: ssh l36 '~/special-agent/scripts/mobile-reauth.sh'" "urgent"
    exit 1
