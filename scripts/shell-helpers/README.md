...
Inspired by Simon Willison's [Running Special Agent in Docker](https://til.simonwillison.net/llms/special-agent-docker).
...
On first command, ClawDock auto-detects your Special Agent directory:

- Checks common paths (`~/special-agent`, `~/workspace/special-agent`, etc.)
...
```bash
special-agent channels login --channel whatsapp --verbose
```
...
2. Configure it in the Special Agent config
...
- Bash or Zsh shell
- Special Agent project (from `docker-setup.sh`)
