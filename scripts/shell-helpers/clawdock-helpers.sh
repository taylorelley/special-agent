#!/usr/bin/env bash
# ClawDock - Docker helpers for Special Agent
# Inspired by Simon Willison's "Running Special Agent in Docker"
# https://til.simonwillison.net/llms/special-agent-docker
#
# Installation:
...
# Common paths to check for Special Agent
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/special-agent"
  "${HOME}/workspace/special-agent"
  "${HOME}/projects/special-agent"
  "${HOME}/dev/special-agent"
  "${HOME}/code/special-agent"
  "${HOME}/src/special-agent"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Special Agent at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/special-agent"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Special Agent not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/special-agent/special-agent.git ~/special-agent"
    echo "  cd ~/special-agent && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^SPECIAL_AGENT_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d special-agent-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart special-agent-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f special-agent-gateway
}
...
clawdock-workspace() {
  cd ~/.special-agent/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec special-agent-gateway \
    bash -c 'echo "alias special-agent=\"./special-agent.mjs\"" > /tmp/.bashrc_special-agent && bash --rcfile /tmp/.bashrc_special-agent'
}
...
clawdock-cli() {
  _clawdock_compose run --rm special-agent-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build special-agent-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "SPECIAL_AGENT_GATEWAY_TOKEN=$token" special-agent-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" special-agent-gateway \
    bash -c './special-agent.mjs config set gateway.remote.token "$TOKEN" && ./special-agent.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec special-agent-gateway \
    bash -c "./special-agent.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart special-agent-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm special-agent-cli dashboard --no-open 2>&1)
  status=$?
...
clawdock-devices() {
  ...
  local output status
  output=$(_clawdock_compose exec special-agent-gateway node dist/index.js devices list 2>&1)
  status=$?
  ...
  if [ $status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'special-agent config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Special Agent${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (special-agent alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to special-agent project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.special-agent)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'special-agent channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'special-agent status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.special-agent.ai${_CLR_RESET}"
  echo ""
}
