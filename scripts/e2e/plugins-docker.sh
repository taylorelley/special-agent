...
IMAGE_NAME="special-agent-plugins-e2e"
...
echo "Running plugins Docker E2E..."
	docker run --rm -t "$IMAGE_NAME" bash -lc '
	  ...
	  if [ -f dist/index.mjs ]; then
	    SPECIAL_AGENT_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    SPECIAL_AGENT_ENTRY="dist/index.js"
	  else
	    echo "Missing dist/index.(m)js (build output):"
	    ls -la dist || true
	    exit 1
	  fi
	  export SPECIAL_AGENT_ENTRY

	  home_dir=$(mktemp -d "/tmp/special-agent-plugins-e2e.XXXXXX")
	  export HOME="$home_dir"
  mkdir -p "$HOME/.special-agent/extensions/demo-plugin"

  cat > "$HOME/.special-agent/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.special-agent/extensions/demo-plugin/special-agent.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$SPECIAL_AGENT_ENTRY" plugins list --json > /tmp/plugins.json
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/special-agent-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@special-agent/demo-plugin-tgz",
  "version": "0.0.1",
...
JS
  cat > "$pack_dir/package/special-agent.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
JSON
  tar -czf /tmp/demo-plugin-tgz.tgz -C "$pack_dir" package
	  ...
	  node "$SPECIAL_AGENT_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
	  node "$SPECIAL_AGENT_ENTRY" plugins list --json > /tmp/plugins2.json
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/special-agent-plugin-dir.XXXXXX")"
...
JS
  cat > "$dir_plugin/special-agent.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$SPECIAL_AGENT_ENTRY" plugins install "$dir_plugin"
	  node "$SPECIAL_AGENT_ENTRY" plugins list --json > /tmp/plugins3.json
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/special-agent-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@special-agent/demo-plugin-file",
  "version": "0.0.1",
  "special-agent": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$file_pack_dir/package/special-agent.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$SPECIAL_AGENT_ENTRY" plugins install "file:$file_pack_dir/package"
	  node "$SPECIAL_AGENT_ENTRY" plugins list --json > /tmp/plugins4.json
