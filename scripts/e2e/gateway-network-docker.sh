...
IMAGE_NAME="special-agent-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="special-agent-net-e2e-$$"
GW_NAME="special-agent-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "SPECIAL_AGENT_GATEWAY_TOKEN=$TOKEN" \
	  -e "SPECIAL_AGENT_SKIP_CHANNELS=1" \
	  -e "SPECIAL_AGENT_SKIP_GMAIL_WATCHER=1" \
	  -e "SPECIAL_AGENT_SKIP_CRON=1" \
	  -e "SPECIAL_AGENT_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
