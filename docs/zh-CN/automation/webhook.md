...
- `special-agent webhooks gmail setup` 为 `special-agent webhooks gmail run` 写入 `hooks.gmail` 配置。完整的 Gmail 监听流程请参阅 [Gmail Pub/Sub](/automation/gmail-pubsub)。
...
curl -X POST http://127.0.0.1:18789/hooks/agent \
  -H 'x-special-agent-token: SECRET' \
  -H 'Content-Type: application/json' \
