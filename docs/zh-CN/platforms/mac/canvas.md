...
- `~/Library/Application Support/Special Agent/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `special-agent-canvas://<session>/<path>`

示例：

- `special-agent-canvas://main/` → `<canvasRoot>/main/index.html`
- `special-agent-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `special-agent-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
special-agent nodes canvas present --node <id>
special-agent nodes canvas navigate --node <id> --url "/"
special-agent nodes canvas eval --node <id> --js "document.title"
special-agent nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__special-agent__/a2ui/
```
...
special-agent nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
special-agent nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "special-agent://agent?message=Review%20this%20design";
```
