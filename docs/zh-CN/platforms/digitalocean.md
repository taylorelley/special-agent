---
read_when:
  - 在 DigitalOcean 上设置 Special Agent
  - 寻找便宜的 VPS 托管来运行 Special Agent
summary: 在 DigitalOcean 上运行 Special Agent（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 Special Agent

## 目标

以 **$6/月**（或使用预留定价 $4/月）在 DigitalOcean 上运行持久的 Special Agent Gateway 网关。
...
- `~/.special-agent/workspace/` — 工作区（SOUL.md、记忆等）
