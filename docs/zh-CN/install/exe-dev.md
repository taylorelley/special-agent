...
summary: 在 exe.dev 上运行 Special Agent Gateway 网关（VM + HTTPS 代理）以实现远程访问
title: exe.dev
x-i18n:
...
目标：Special Agent Gateway 网关运行在 exe.dev VM 上，可从你的笔记本电脑通过以下地址访问：`https://<vm-name>.exe.xyz`
...
提示：保持此 VM **有状态**。Special Agent 在 `~/.special-agent/` 和 `~/.special-agent/workspace/` 下存储状态。
...
## 5) 访问 Special Agent 并授予权限

访问 `https://<vm-name>.exe.xyz/?token=YOUR-TOKEN-FROM-TERMINAL`（参阅新手引导中的控制 UI 输出）。使用 `special-agent devices list` 和 `special-agent devices approve <requestId>` 批准设备。如有疑问，从浏览器使用 Shelley！
