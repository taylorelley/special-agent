...
当你使用 `--channel` **显式**切换渠道时，Special Agent 还会对齐安装方式：

- `dev` 确保有一个 git checkout（默认 `~/special-agent`，可通过 `SPECIAL_AGENT_GIT_DIR` 覆盖），
...
当你使用 `special-agent update` 切换渠道时，Special Agent 还会同步插件来源：
