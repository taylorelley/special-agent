...
你可以在 `~/.special-agent/special-agent.json` 中覆盖此设置：
...
所有日志配置都在 `~/.special-agent/special-agent.json` 的 `logging` 下。
...
- Special Agent 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `special-agent plugins enable diagnostics-otel` 启用插件。
...
- `special-agent.tokens`（计数器，属性：`special-agent.token`、`special-agent.channel`、`special-agent.provider`、`special-agent.model`）
- `special-agent.cost.usd`（计数器，属性：`special-agent.channel`、`special-agent.provider`、`special-agent.model`）
- `special-agent.run.duration_ms`（直方图，属性：`special-agent.channel`、`special-agent.provider`、`special-agent.model`）
- `special-agent.context.tokens`（直方图，属性：`special-agent.context`、`special-agent.channel`、`special-agent.provider`、`special-agent.model`）

消息流：

- `special-agent.webhook.received`（计数器，属性：`special-agent.channel`、`special-agent.webhook`）
- `special-agent.webhook.error`（计数器，属性：`special-agent.channel`、`special-agent.webhook`）
- `special-agent.webhook.duration_ms`（直方图，属性：`special-agent.channel`、`special-agent.webhook`）
- `special-agent.message.queued`（计数器，属性：`special-agent.channel`、`special-agent.source`）
- `special-agent.message.processed`（计数器，属性：`special-agent.channel`、`special-agent.outcome`）
- `special-agent.message.duration_ms`（直方图，属性：`special-agent.channel`、`special-agent.outcome`）

队列 + 会话：

- `special-agent.queue.lane.enqueue`（计数器，属性：`special-agent.lane`）
- `special-agent.queue.lane.dequeue`（计数器，属性：`special-agent.lane`）
- `special-agent.queue.depth`（直方图，属性：`special-agent.lane` 或 `special-agent.channel=heartbeat`）
- `special-agent.queue.wait_ms`（直方图，属性：`special-agent.lane`）
- `special-agent.session.state`（计数器，属性：`special-agent.state`、`special-agent.reason`）
- `special-agent.session.stuck`（计数器，属性：`special-agent.state`）
- `special-agent.session.stuck_age_ms`（直方图，属性：`special-agent.state`）
- `special-agent.run.attempt`（计数器，属性：`special-agent.attempt`）
...
- `special-agent.webhook.error`
  - `special-agent.channel`、`special-agent.webhook`、`special-agent.chatId`、`special-agent.error`
- `special-agent.message.processed`
  - `special-agent.channel`、`special-agent.outcome`、`special-agent.chatId`、`special-agent.messageId`、`special-agent.sessionKey`、`special-agent.sessionId`、`special-agent.reason`
- `special-agent.session.stuck`
  - `special-agent.state`、`special-agent.ageMs`、`special-agent.queueDepth`、`special-agent.sessionKey`、`special-agent.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `special-agent doctor`。
