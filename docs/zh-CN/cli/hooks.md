...
read_when:
  - 你想管理智能体钩子
  - 你想安装或更新钩子
summary: CLI 参考：`special-agent hooks`（智能体钩子）
title: hooks
x-i18n:
...
通过将特定钩子添加到配置（`~/.special-agent/config.json`）来启用它。

**注意：** 由插件管理的钩子在 `special-agent hooks list` 中显示 `plugin:<id>`，
...
- 将钩子包复制到 `~/.special-agent/hooks/<id>`
...
**输出：** `~/.special-agent/workspace/memory/YYYY-MM-DD-slug.md`
