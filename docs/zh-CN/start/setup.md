...
summary: 设置指南：在保持最新的同时保持你的 Special Agent 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.special-agent/workspace`（工作区）+ `~/.special-agent/special-agent.json`（配置）。
...
- **配置：** `~/.special-agent/special-agent.json`（JSON/JSON5 格式）
- **工作区：** `~/.special-agent/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm special-agent setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **SpecialAgent.app**（菜单栏）。
...
- 运行 `special-agent setup`，然后 `special-agent channels login`，然后手动启动 Gateway 网关（`special-agent gateway`）。
...
- **配对允许列表**：`~/.special-agent/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.special-agent/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.special-agent/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.special-agent/workspace` 和 `~/.special-agent/` 保持为"你的东西"；不要将个人提示/配置放入 `special-agent` 仓库。
...
- [Special Agent 助手设置](/start/special-agent)
