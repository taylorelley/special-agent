...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.special-agent/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.special-agent/node.json` (node id + pairing token).
...
Path: `~/.special-agent/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.special-agent/exec-approvals.sock",
...
- Unix socket at `~/.special-agent/exec-approvals.sock` (0600).
