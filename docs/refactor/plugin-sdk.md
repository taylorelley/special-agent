...
- Publish as `special-agent/plugin-sdk` (or export from core under `special-agent/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `Special AgentPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: SpecialAgentConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: SpecialAgentConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: SpecialAgentConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: SpecialAgentConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: SpecialAgentConfig): string;
  };
};
```
...
- Introduce `special-agent/plugin-sdk`.
- Add `api.runtime` to `Special AgentPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `specialAgentRuntime: ">=2026.2.0"`).
