...
- Plugin manifests (`special-agent.plugin.json`) are required for all plugins.

## Plugin schema enforcement

- Each plugin provides a strict JSON Schema for its config (inline in the manifest).
- Plugin load flow:
  1. Resolve plugin manifest + schema (`special-agent.plugin.json`).
...
- If config invalid:
  ...
  - Instruct: `special-agent doctor --fix`.
...
Everything else must hard-fail with: “Config invalid. Run `special-agent doctor --fix`.”
