...
read_when:
  - You need detailed behavior for special-agent onboard
...
This page is the full reference for `special-agent onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.special-agent/special-agent.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `special-agent doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `special-agent pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `special-agent health`.
    - `special-agent status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.special-agent/.env` so launchd can read it.
...
- OAuth credentials: `~/.special-agent/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.special-agent/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.special-agent/credentials/oauth.json` (or `$SPECIAL_AGENT_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`special-agent onboard`](/cli/onboard)
