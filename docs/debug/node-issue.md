...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Special Agent
...
Running Special Agent via Node with `tsx` fails at startup with:

```
[special-agent] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `special-agent status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch special-agent.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node special-agent.mjs status` works on Node 25.
