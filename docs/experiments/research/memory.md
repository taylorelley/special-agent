...
read_when:
  - Designing workspace memory (~/.special-agent/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Special Agent integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.special-agent/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.special-agent/workspace` as canonical human-readable memory.
...
```
~/.special-agent/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Special Agent**, but keep a separable core library.

### Why integrate into Special Agent?

- Special Agent already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `special-agent memory recall "…" --k 25 --since 30d`
  - `special-agent memory reflect --since 7d`
...
Pragmatic take for `~/.special-agent/workspace`:
