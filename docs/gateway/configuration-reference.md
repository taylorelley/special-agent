...
description: "Complete field-by-field reference for ~/.special-agent/special-agent.json"
---

# Configuration Reference

Every field available in `~/.special-agent/special-agent.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Special Agent uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `special-agent doctor` into `whatsapp/default`.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupEnabled: false,
        groupChannels: ["special-agent-dm"],
      },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `special-agent plugins install @special-agent/mattermost`.
...
Special Agent spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@special-agent"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `special-agent.json`).
...
Default: `~/.special-agent/workspace`.
...
Optional repository root shown in the system prompt's Runtime line. If unset, Special Agent auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Special Agent assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.special-agent/sandboxes`
    ...
    list: [
      {
        ...
        agentDir: "~/.special-agent/agents/main/agent",
...
Special Agent uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.special-agent/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `SPECIAL_AGENT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `special-agent onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `special-agent onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `special-agent onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `special-agent onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `special-agent onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `special-agent onboard --auth-choice minimax-api`.
...
- Loaded from `~/.special-agent/extensions`, `<workspace>/.special-agent/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or SPECIAL_AGENT_GATEWAY_PASSWORD
      allowTailscale: true,
    },
    tailscale: {
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `SPECIAL_AGENT_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.special-agent-dev` + port `19001`), `--profile <name>` (uses `~/.special-agent-<name>`).
...
Auth: `Authorization: Bearer <token>` or `x-special-agent-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `SPECIAL_AGENT_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    port: 18793,
    liveReload: true,
    // enabled: false, // or SPECIAL_AGENT_SKIP_CANVAS_HOST=1
  },
}
```
...
- Also serves A2UI at `/__special-agent__/a2ui/`.
...
- Hostname defaults to `special-agent`. Override with `SPECIAL_AGENT_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.special-agent/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `special-agent dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.special-agent/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${SPECIAL_AGENT_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.special-agent/credentials/oauth.json`.
...
- Default log file: `/tmp/special-agent/special-agent-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `special-agent doctor --fix` can strip unknown keys).
