...
`special-agent doctor` is the repair + migration tool for Special Agent. It fixes stale
...
```bash
special-agent doctor --yes
```

Accept defaults without prompting (including restart/service/sandbox repair steps when applicable).

```bash
special-agent doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
special-agent doctor --repair --force
```
...
```bash
cat ~/.special-agent/special-agent.json
```
...
- Extra workspace dir detection (`~/special-agent`).
...
you to run `special-agent doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.special-agent/special-agent.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.special-agent/sessions/` to `~/.special-agent/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.special-agent/agent/` to `~/.special-agent/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.special-agent/credentials/*.json` (except `oauth.json`)
  - to `~/.special-agent/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `special-agent doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.special-agent` folders exist across
  home directories or when `SPECIAL_AGENT_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.special-agent/special-agent.json` is
...
offers to remove them and install the Special Agent service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Special Agent gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `special-agent doctor --generate-gateway-token` to force token
...
- `special-agent doctor` prompts before rewriting supervisor config.
- `special-agent doctor --yes` accepts the default repair prompts.
- `special-agent doctor --repair` applies recommended fixes without prompts.
- `special-agent doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `special-agent gateway install --force`.
