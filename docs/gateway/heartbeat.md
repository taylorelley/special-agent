...
- During heartbeat runs, Special Agent treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `special-agent sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Special Agent skips the heartbeat run entirely (no model call).
...
```bash
special-agent system event --text "Check for urgent follow-ups" --mode now
```
