...
Special Agent’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`special-agent agent`), so routing/permissions/config match your Gateway.
