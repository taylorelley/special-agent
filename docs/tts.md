...
Special Agent can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere Special Agent can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, Special Agent defaults
...
TTS config lives under `messages.tts` in `special-agent.json`.
...
- If `provider` is **unset**, Special Agent prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.special-agent/settings/tts.json`, override with `SPECIAL_AGENT_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, Special Agent retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, Special Agent:
...
Discord note: `/tts` is a built-in Discord command, so Special Agent registers
...
/tts limit 2000
/tts summary off
/tts audio Hello from Special Agent
```
