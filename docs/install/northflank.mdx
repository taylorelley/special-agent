...
Deploy Special Agent on Northflank with a one-click template and finish setup in your browser.
...
1. Click [Deploy Special Agent](https://northflank.com/stacks/deploy-special-agent) to open the template.
2. Create an [account on Northflank](https://app.northflank.com/signup) if you don’t already have one.
3. Click **Deploy Special Agent now**.
4. Set the required environment variable: `SETUP_PASSWORD`.
5. Click **Deploy stack** to build and run the Special Agent template.
6. Wait for the deployment to complete, then click **View resources**.
7. Open the Special Agent service.
8. Open the public Special Agent URL and complete setup at `/setup`.
9. Open the Control UI at `/special-agent`.
...
4. Click **Run setup**.
5. Open the Control UI at `https://<your-northflank-domain>/special-agent`
