...
Deploy Special Agent on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/special-agent-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/special-agent` — Control UI

## What you get

- Hosted Special Agent Gateway + Control UI
...
- `SPECIAL_AGENT_STATE_DIR=/data/.special-agent` (recommended)
- `SPECIAL_AGENT_WORKSPACE_DIR=/data/workspace` (recommended)
- `SPECIAL_AGENT_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Special Agent state + workspace so you can migrate to another host without losing config or memory.
