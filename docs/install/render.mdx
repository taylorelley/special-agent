...
Deploy Special Agent on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/special-agent/special-agent)
...
repository configures everything needed to run Special Agent:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: SPECIAL_AGENT_STATE_DIR
        value: /data/.special-agent
      - key: SPECIAL_AGENT_WORKSPACE_DIR
        value: /data/workspace
      - key: SPECIAL_AGENT_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: special-agent-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/special-agent`.
...
If you use the original Special Agent repository, Render will not auto-deploy your Special Agent. To update it, run a manual Blueprint sync from the dashboard.
...
For Special Agent, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Special Agent host.
