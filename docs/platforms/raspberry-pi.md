---
summary: "Special Agent on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Special Agent on a Raspberry Pi
  - Running Special Agent on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Special Agent on Raspberry Pi

## Goal

Run a persistent, always-on Special Agent Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Special Agent features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled special-agent

# Enable if not
sudo systemctl enable special-agent

# Start on boot
sudo systemctl start special-agent
```
...
```bash
# Check logs
journalctl -u special-agent --no-pager -n 100

# Common fix: rebuild
cd ~/special-agent  # if using hackable install
npm run build
sudo systemctl restart special-agent
```
