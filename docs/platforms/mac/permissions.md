...
- Same path: run the app from a fixed location (for Special Agent, `dist/SpecialAgent.app`).
...
Workaround: move files into the Special Agent workspace (`~/.special-agent/workspace`) if you want to avoid per-folder grants.
