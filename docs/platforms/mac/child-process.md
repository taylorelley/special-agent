...
service via the external `special-agent` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`SPECIAL_AGENT_PROFILE`; legacy `com.special-agent.*` is supported).
