...
This document explains how Special Agent manages sessions end-to-end:
...
Special Agent is designed around a single **Gateway process** that owns session state.
...
Special Agent persists sessions in two layers:
...
- Transcripts: `~/.special-agent/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Special Agent resolves these via `src/config/sessions.ts`.
...
Special Agent intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Special Agent consumes the events, but Pi decides when to compact).
...
Special Agent also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Special Agent bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Special Agent leaves it alone.
...
- `special-agent status` (CLI)
- `special-agent sessions` / `sessions --json`
...
Special Agent supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Special Agent strips/suppresses this in the delivery layer.

As of `2026.1.10`, Special Agent also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Special Agent uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Special Agent’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `special-agent status`.
