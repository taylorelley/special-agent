...
Special Agent integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Special Agent owns lifecycle when `channels.signal.autoStart=true`.
...
- Special Agent spawns `imsg rpc` as a child process (legacy iMessage integration).
