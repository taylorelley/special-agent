...
Special Agent features that can generate provider usage or paid API calls.
...
- `special-agent status --usage` and `special-agent channels list` show provider **usage windows**
...
Special Agent can pick up credentials from:
...
- `special-agent status --usage`
- `special-agent models status --json`
...
`special-agent models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
