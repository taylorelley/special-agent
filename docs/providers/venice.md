---
summary: "Use Venice AI privacy-focused models in Special Agent"
read_when:
  - You want privacy-focused inference in Special Agent
...
```bash
special-agent onboard --auth-choice venice-api-key
```
...
```bash
special-agent chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Special Agent shows all available Venice models. Pick based on your needs:
...
```bash
special-agent models set venice/claude-opus-45
special-agent models set venice/llama-3.3-70b
```

List all available models:

```bash
special-agent models list | grep venice
```

## Configure via `special-agent configure`

1. Run `special-agent configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Special Agent automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
special-agent chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
special-agent chat --model venice/claude-opus-45

# Use uncensored model
special-agent chat --model venice/venice-uncensored

# Use vision model with image
special-agent chat --model venice/qwen3-vl-235b-a22b

# Use coding model
special-agent chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `special-agent models list` to see currently available models. Some models may be temporarily offline.
