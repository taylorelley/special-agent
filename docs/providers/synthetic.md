---
summary: "Use Synthetic's Anthropic-compatible API in Special Agent"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Special Agent registers it as the
...
Note: Special Agent's Anthropic client appends `/v1` to the base URL, so use
