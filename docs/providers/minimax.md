---
summary: "Use MiniMax M2.1 in Special Agent"
read_when:
  - You want MiniMax models in Special Agent
...
```bash
special-agent plugins enable minimax-portal-auth  # skip if already loaded.
special-agent gateway restart  # restart if gateway is already running
special-agent onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/special-agent/special-agent/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `special-agent.json`:
...
- Use `special-agent models list` and `special-agent models set minimax/MiniMax-M2.1` to switch.
...
- Running `special-agent configure` and selecting **MiniMax M2.1**, or
