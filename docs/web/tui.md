...
```bash
special-agent tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `special-agent tui --deliver`
...
- Check the Gateway logs: `special-agent logs --follow`.
- Confirm the agent can run: `special-agent status` and `special-agent models status`.
...
- No agents in picker: check `special-agent agents list` and your routing config.
