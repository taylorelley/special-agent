...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/special-agent`)
...
If the page fails to load, start the Gateway first: `special-agent gateway`.
...
you revoke it with `special-agent devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.special-agent/special-agent.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Special Agent
...
Special Agent **blocks** Control UI connections without device identity.
