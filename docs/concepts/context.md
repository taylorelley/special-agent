...
read_when:
  - You want to understand what “context” means in Special Agent
...
“Context” is **everything Special Agent sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Special Agent-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Special Agent builds the system prompt

The system prompt is **Special Agent-owned** and rebuilt each run. It includes:
...
By default, Special Agent injects a fixed set of workspace files (if present):
