...
- `/usage cost` in chats: local cost summary aggregated from Special Agent session logs.
- CLI: `special-agent status --usage` prints a full per-provider breakdown.
- CLI: `special-agent channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
