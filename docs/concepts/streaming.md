...
Special Agent has two separate “streaming” layers:
...
When block streaming is enabled, Special Agent can **merge consecutive block chunks**
...
When draft streaming is active, Special Agent disables block streaming for that reply to avoid double-streaming.
