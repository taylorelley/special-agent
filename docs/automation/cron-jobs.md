...
- Jobs persist under `~/.special-agent/cron/` so restarts don’t lose schedules.
...
special-agent cron list
special-agent cron run <job-id>
special-agent cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.special-agent/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `special-agent cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, Special Agent defaults to `announce`.
...
- Job store: `~/.special-agent/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.special-agent/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.special-agent/cron/jobs.json",
...
special-agent cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
special-agent cron edit <jobId> --agent ops
special-agent cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
special-agent cron run <jobId>
special-agent cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
special-agent cron edit <jobId> \
...
```bash
special-agent system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`special-agent system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `SPECIAL_AGENT_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Special Agent applies exponential retry backoff for recurring jobs after consecutive errors:
