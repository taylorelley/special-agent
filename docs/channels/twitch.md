...
read_when:
  - Setting up Twitch chat integration for Special Agent
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Special Agent connects as a Twitch user (bot account) to receive and send messages in channels.
...
4. Configure the token:
   - Env: `SPECIAL_AGENT_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "special-agent", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SPECIAL_AGENT_TWITCH_ACCESS_TOKEN env var)
...
```bash
SPECIAL_AGENT_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
