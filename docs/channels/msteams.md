...
```bash
special-agent plugins install @special-agent/msteams
```

Local checkout (when running from a git repo):

```bash
special-agent plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Special Agent will offer the local install path automatically.
...
3. Configure Special Agent with those credentials.
...
- Talk to Special Agent via Teams DMs, group chats, or channels.
...
- On startup, Special Agent resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.special-agent/special-agent.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Special Agent, you need to create an Azure Bot resource.
...
1. **Install the Microsoft Teams plugin**
   - From npm: `special-agent plugins install @special-agent/msteams`
   - From a local checkout: `special-agent plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Special Agent in Teams", "full": "Special Agent in Teams" },
...
Special Agent handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Special Agent only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/Special AgentShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Special Agent sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `special-agent message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.special-agent/msteams-polls.json`.
...
```bash
special-agent message send --channel msteams \
...
special-agent message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
special-agent message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
special-agent message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
special-agent message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
