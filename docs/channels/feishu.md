...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Special Agent to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
special-agent plugins install @special-agent/feishu
```

Local checkout (when running from a git repo):

```bash
special-agent plugins install ./extensions/feishu
```
...
If you just installed Special Agent, run the wizard:
...
2. Configuring app credentials in Special Agent
...
1. You already ran `special-agent channels add` for Feishu
2. The gateway is running (`special-agent gateway status`)
...
## Step 2: Configure Special Agent
...
Edit `~/.special-agent/special-agent.json`:
...
```bash
special-agent pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  special-agent pairing list feishu
  special-agent pairing approve feishu <CODE>
  ```
...
2. Run `special-agent logs --follow` and look for `chat_id`
...
2. Run `special-agent logs --follow` and look for `open_id`
...
| `special-agent gateway restart` | Restart gateway service       |
| `special-agent logs --follow`   | Tail gateway logs             |
...
4. Check logs: `special-agent logs --follow`
...
5. Ensure the gateway is running: `special-agent gateway status`
6. Check logs: `special-agent logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.special-agent/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.special-agent/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
