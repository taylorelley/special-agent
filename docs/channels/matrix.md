...
Matrix is an open, decentralized messaging protocol. Special Agent connects as a Matrix **user**
...
```bash
special-agent plugins install @special-agent/matrix
```

Local checkout (when running from a git repo):

```bash
special-agent plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `special-agent plugins install @special-agent/matrix`
   - From a local checkout: `special-agent plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Special Agent calls the same
     login endpoint, stores the access token in `~/.special-agent/credentials/matrix/credentials.json`,
...
- On first connection, Special Agent requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Special Agent logs a warning.
...
`~/.special-agent/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `special-agent pairing list matrix`
  - `special-agent pairing approve matrix <CODE>`
...
- On startup, Special Agent resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
