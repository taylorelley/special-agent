...
```bash
special-agent plugins install @special-agent/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
special-agent plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Special Agent" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `special-agent pairing list nextcloud-talk`
  - `special-agent pairing approve nextcloud-talk <CODE>`
