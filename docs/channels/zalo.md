...
- Install via CLI: `special-agent plugins install @special-agent/zalo`
...
1. Install the Zalo plugin:
   - From a source checkout: `special-agent plugins install ./extensions/zalo`
   - From npm (if published): `special-agent plugins install @special-agent/zalo`
...
- Approve via:
  - `special-agent pairing list zalo`
  - `special-agent pairing approve zalo <CODE>`
...
- Example: `special-agent message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `special-agent channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `special-agent logs --follow`
