---
title: IRC
description: Connect Special Agent to IRC channels and direct messages.
---

Use IRC when you want Special Agent in classic channels (`#room`) and direct messages.
IRC ships as an extension plugin, but it is configured in the main config under `channels.irc`.

## Quick start

1. Enable IRC config in `~/.special-agent/special-agent.json`.
2. Set at least:
...
```json
{
  "channels": {
    "irc": {
      ...
      "port": 6697,
      "tls": true,
      "nick": "special-agent-bot",
      "channels": ["#special-agent"]
    }
  }
}
```
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, Special Agent defaults to **mention-gating** in group contexts.
