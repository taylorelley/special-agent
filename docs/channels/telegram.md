...
<Tabs>
  <Tab title="DM policy">
    ...
    1. DM your bot.
    2. Run `special-agent logs --follow`.
    3. Read `from.id`.
...
```
    ...
    - or read `chat.id` from `special-agent logs --follow`
...
- DM messages can carry `message_thread_id`; Special Agent routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Draft streaming in Telegram DMs">
    Special Agent can stream partial replies with Telegram draft bubbles (`sendMessageDraft`).
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, Special Agent retries as plain text.
...
```
    ...
    - `~/.special-agent/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, Special Agent enqueues system events like:
...
```bash
special-agent message send --channel telegram --target 123456789 --message "hi"
special-agent message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `special-agent channels status` warns when config expects unmentioned group messages.
    - `special-agent channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `special-agent logs --follow` for skip reasons
