...
read_when:
  - You want Special Agent to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Special Agent to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`special-agent onboard`) and `special-agent channels add` list optional channel plugins.
...
```bash
special-agent plugins install @special-agent/nostr
```

Use a local checkout (dev workflows):

```bash
special-agent plugins install --link <path-to-special-agent>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "special-agent@example.com",
        "lud16": "special-agent@example.com"
      }
    }
  }
}
