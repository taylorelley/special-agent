...
Tlon is a decentralized messenger built on Urbit. Special Agent connects to your Urbit ship and can
...
```bash
special-agent plugins install @special-agent/tlon
```

Local checkout (when running from a git repo):

```bash
special-agent plugins install ./extensions/tlon
```
...
Use these with `special-agent message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Special Agent replies in-thread.
