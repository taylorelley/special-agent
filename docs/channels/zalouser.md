...
read_when:
  - Setting up Zalo Personal for Special Agent
...
- Install via CLI: `special-agent plugins install @special-agent/zalouser`
- Or from a source checkout: `special-agent plugins install ./extensions/zalouser`
...
- `special-agent pairing list zalouser`
- `special-agent pairing approve zalouser <code>`
...
- On startup, Special Agent resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- Re-login: `special-agent channels logout --channel zalouser && special-agent channels login --channel zalouser`
