---
title: Lobster
summary: "Typed workflow runtime for Special Agent with resumable approval gates."
description: Typed workflow runtime for Special Agent — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets Special Agent run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Special Agent runs one Lobster tool call and gets a structured result.
...
Special Agent launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | special-agent.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
special-agent.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Special Agent tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Special Agent Gateway (see the [Lobster repo](https://github.com/special-agent/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Special Agent keeps core tools enabled. To restrict core
...
→ special-agent calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ special-agent calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Special Agent tools that do.
