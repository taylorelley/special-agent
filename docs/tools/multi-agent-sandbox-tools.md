...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `special-agent sandbox explain`.
...
Legacy `agent.*` configs are migrated by `special-agent doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=special-agent-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${SPECIAL_AGENT_STATE_DIR:-$HOME/.special-agent}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
