---
summary: "Chrome extension: let Special Agent drive your existing Chrome tab"
read_when:
...
The Special Agent Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate special-agent-managed Chrome profile.
...
Special Agent then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Special Agent release (npm package) as static files. There is no separate “build” step.

After upgrading Special Agent:

- Re-run `special-agent browser extension install` to refresh the installed files under your Special Agent state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Special Agent ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Special Agent to control.
...
- `ON`: attached; Special Agent can drive that tab.
...
Debugging: `special-agent sandbox explain`
...
`special-agent browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `special-agent browser extension install` first to copy the extension to a stable location under your Special Agent state directory.
...
- **This is not isolated** like the dedicated special-agent-managed profile.
