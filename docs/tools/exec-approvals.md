...
- **gateway host** → `special-agent` process on the gateway machine
...
`~/.special-agent/exec-approvals.json` directly.

CLI: `special-agent approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
