...
// Mock loadConfig so resolveMaintenanceConfig() never reads a real special-agent.json.
...
describe("rotateSessionFile", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-rotate-"));
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-pruning-integ-"));
    storePath = path.join(testDir, "sessions.json");
    savedCacheTtl = process.env.SPECIAL_AGENT_SESSION_CACHE_TTL_MS;
    process.env.SPECIAL_AGENT_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  ...
  afterEach(async () => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.SPECIAL_AGENT_SESSION_CACHE_TTL_MS;
    } else {
      process.env.SPECIAL_AGENT_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
