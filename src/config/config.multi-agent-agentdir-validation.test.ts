...
describe("multi-agent agentDir validation", () => {
  it("rejects shared agents.list agentDir", async () => {
    ...
    const shared = path.join(tmpdir(), "special-agent-shared-agentdir");
        ...
        JSON.stringify(
          {
            agents: {
              list: [
                { id: "a", agentDir: "~/.special-agent/agents/shared/agent" },
                { id: "b", agentDir: "~/.special-agent/agents/shared/agent" },
              ],
            },
