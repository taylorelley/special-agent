...
const ETC_SPECIAL_AGENT_DIR = path.join(ROOT_DIR, "etc", "special-agent");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "special-agent.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcSpecialAgentPath(...parts: string[]) {
  return path.join(ETC_SPECIAL_AGENT_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcSpecialAgentPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "special-agent.json"))).toEqual({
      shared: true,
    });
  });
});
