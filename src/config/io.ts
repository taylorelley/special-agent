...
import type { SpecialAgentConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareSpecialAgentVersions } from "./version.js";
...
function coerceConfig(value: unknown): SpecialAgentConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as SpecialAgentConfig;
}
...
function stampConfigVersion(cfg: SpecialAgentConfig): SpecialAgentConfig {
...
function warnIfConfigFromFuture(cfg: SpecialAgentConfig, logger: Pick<typeof console, "warn">): void {
  ...
  const cmp = compareSpecialAgentVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer SpecialAgent (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: SpecialAgentConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): SpecialAgentConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as SpecialAgentConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as SpecialAgentConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: SpecialAgentConfig) {
    clearConfigCache();
...
// module scope. `SPECIAL_AGENT_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.SPECIAL_AGENT_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.SPECIAL_AGENT_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): SpecialAgentConfig {
...
export async function writeConfigFile(cfg: SpecialAgentConfig): Promise<void> {
  clearConfigCache();
