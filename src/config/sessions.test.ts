...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-special-agent",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-special-agent#general");
  });
  ...
  it("derives session transcripts dir from SPECIAL_AGENT_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { SPECIAL_AGENT_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.SPECIAL_AGENT_STATE_DIR = "/custom/state";
    try {
