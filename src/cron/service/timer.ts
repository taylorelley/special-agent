 ...
 * but always breaks an infinite re-trigger cycle.  (See openclaw#17821)
 */
...
function resolveCronJobTimeoutMs(job: CronJob): number {
  if (job.payload.kind === "agentTurn" && typeof job.payload.timeoutSeconds === "number") {
    return job.payload.timeoutSeconds * 1_000;
  }
...
function resolveDeliveryStatus(params: { job: CronJob; delivered?: boolean }) {
  ...
  if (!shouldDelete) {
    if (job.schedule.kind === "at") {
      ...
      if (job.schedule.kind === "cron") {
        ...
        // timezone/croner edge cases (see openclaw#17821).
  ...
  try {
    const dueJobs = await locked(state, async () => {
      ...
      if (due.length === 0) {
        ...
        // when the timer wakes up but findDueJobs returns empty (see openclaw#13992).
    ...
    if (completedResults.length > 0) {
      await locked(state, async () => {
        ...
        // daily cron schedules to jump 48 h instead of 24 h (openclaw#17852).
...
function findDueJobs(state: CronServiceState): CronJob[] {
  ...
  return state.store.jobs.filter((j) => {
    ...
    return typeof next === "number" && Number.isFinite(next) && now >= next;
  });
}
    ...
    if (job.wakeMode === "now" && state.deps.runHeartbeatOnce) {
      ...
      for (;;) {
        ...
        heartbeatResult = await state.deps.runHeartbeatOnce({
          ...
          // Without this override, heartbeat target defaults to "none" and
          // cron main-session responses are silently swallowed.
...
export async function executeJob(
  ...
  // Persist so that callers that force-reload from disk (e.g. start())
  // pick up the updated job state.
  await persist(state);
}
