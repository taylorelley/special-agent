...
import type { SpecialAgentConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: SpecialAgentConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("special-agent security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("special-agent configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("special-agent config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("special-agent doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("special-agent config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
