...
describe("onboard (non-interactive): LiteLLM", () => {
  it("stores the API key and configures the default model", async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-onboard-litellm-"));
    process.env.HOME = tempHome;
    process.env.SPECIAL_AGENT_STATE_DIR = tempHome;
    process.env.SPECIAL_AGENT_CONFIG_PATH = path.join(tempHome, "special-agent.json");
    vi.resetModules();
