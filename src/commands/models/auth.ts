...
import { readConfigFileSnapshot, type SpecialAgentConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: SpecialAgentConfig, model: string): SpecialAgentConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("special-agent plugins install")}\`.`,
    );
  }
