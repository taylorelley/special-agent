...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveSpecialAgentAgentDir: vi.fn().mockReturnValue("/tmp/special-agent-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/special-agent-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveSpecialAgentAgentDir: mocks.resolveSpecialAgentAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveSpecialAgentAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/special-agent-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/special-agent-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/special-agent-agent-custom");
