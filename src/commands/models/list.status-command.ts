...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveSpecialAgentAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("special-agent models auth setup-token")}\` or \`${formatCliCommand("special-agent configure")}\`.`
          : `Run \`${formatCliCommand("special-agent configure")}\` or set an API key env var.`;
