...
export function listAgentEntries(cfg: SpecialAgentConfig): AgentEntry[] {
...
function resolveAgentName(cfg: SpecialAgentConfig, agentId: string) {
...
function resolveAgentModel(cfg: SpecialAgentConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: SpecialAgentConfig): AgentSummary[] {
