...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.SPECIAL_AGENT_DEBUG_HEALTH)) {
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: SpecialAgentConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.SPECIAL_AGENT_DEBUG_HEALTH);
    if (opts.verbose) {
