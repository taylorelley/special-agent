...
function captureEnv(): EnvSnapshot {
  return {
    ...
    stateDir: process.env.SPECIAL_AGENT_STATE_DIR,
    ...
    password: process.env.SPECIAL_AGENT_GATEWAY_PASSWORD,
    disableConfigCache: process.env.SPECIAL_AGENT_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("SPECIAL_AGENT_STATE_DIR", prev.stateDir);
  restoreEnvVar("SPECIAL_AGENT_CONFIG_PATH", prev.configPath);
  restoreEnvVar("SPECIAL_AGENT_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("SPECIAL_AGENT_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("SPECIAL_AGENT_SKIP_CRON", prev.skipCron);
  restoreEnvVar("SPECIAL_AGENT_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("SPECIAL_AGENT_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("SPECIAL_AGENT_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("SPECIAL_AGENT_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  process.env.SPECIAL_AGENT_DISABLE_CONFIG_CACHE = "1";
  ...
  const configPath = path.join(tempHome, "special-agent.json");
  process.env.HOME = tempHome;
  process.env.SPECIAL_AGENT_STATE_DIR = tempHome;
...
describe("onboard (non-interactive): provider auth", () => {
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("special-agent-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("special-agent-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("special-agent-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
