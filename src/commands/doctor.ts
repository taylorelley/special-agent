...
function resolveMode(cfg: SpecialAgentConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("SpecialAgent doctor");

  const root = await resolveSpecialAgentPackageRoot({
  ...
  let cfg: SpecialAgentConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("special-agent configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("special-agent config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("special-agent setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("special-agent doctor --fix")}" to apply changes.`);
  }
