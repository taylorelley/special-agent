...
beforeAll(() => {
  previousProfile = process.env.SPECIAL_AGENT_PROFILE;
  process.env.SPECIAL_AGENT_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.SPECIAL_AGENT_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/special-agent-root.js", () => ({
  resolveSpecialAgentPackageRoot: vi.fn().mockResolvedValue("/tmp/special-agent"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/special-agent/pnpm-lock.yaml",
      markerPath: "/tmp/special-agent/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("SpecialAgent status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("special-agent status --all") ||
          l.includes("special-agent --profile isolated status --all") ||
          l.includes("special-agent status --all") ||
          l.includes("special-agent --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.SPECIAL_AGENT_GATEWAY_TOKEN = "abcd1234";
    try {
