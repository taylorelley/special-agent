...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    skipBrowser: process.env.SPECIAL_AGENT_SKIP_BROWSER_CONTROL_SERVER,
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.SPECIAL_AGENT_STATE_DIR = prev.stateDir;
    ...
    process.env.SPECIAL_AGENT_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
    process.env.SPECIAL_AGENT_GATEWAY_TOKEN = prev.token;
    process.env.SPECIAL_AGENT_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "special-agent");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.SPECIAL_AGENT_STATE_DIR = stateDir;
    process.env.SPECIAL_AGENT_CONFIG_PATH = path.join(stateDir, "special-agent.json");
