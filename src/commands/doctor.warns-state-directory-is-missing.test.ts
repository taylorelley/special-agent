...
beforeEach(() => {
  ...
  resolveSpecialAgentPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.SPECIAL_AGENT_STATE_DIR;
  originalUpdateInProgress = process.env.SPECIAL_AGENT_UPDATE_IN_PROGRESS;
  process.env.SPECIAL_AGENT_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "special-agent-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.SPECIAL_AGENT_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.SPECIAL_AGENT_UPDATE_IN_PROGRESS;
  } else {
    process.env.SPECIAL_AGENT_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveSpecialAgentPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadSpecialAgentPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/special-agent-root.js", () => ({
  resolveSpecialAgentPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "special-agent-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.SPECIAL_AGENT_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when SPECIAL_AGENT_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/special-agent.json",
      exists: true,
      raw: "{}",
    ...
    process.env.SPECIAL_AGENT_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
