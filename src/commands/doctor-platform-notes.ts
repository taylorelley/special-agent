...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".special-agent", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: SpecialAgentConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const deprecatedLaunchctlEntries = [
    ["SPECIAL_AGENT_GATEWAY_TOKEN", await getenv("SPECIAL_AGENT_GATEWAY_TOKEN")],
    ["SPECIAL_AGENT_GATEWAY_PASSWORD", await getenv("SPECIAL_AGENT_GATEWAY_PASSWORD")],
    ["SPECIAL_AGENT_GATEWAY_TOKEN", await getenv("SPECIAL_AGENT_GATEWAY_TOKEN")],
    ["SPECIAL_AGENT_GATEWAY_PASSWORD", await getenv("SPECIAL_AGENT_GATEWAY_PASSWORD")],
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) =>
          `- \`${key}\` is set; use \`SPECIAL_AGENT_${key.slice(key.indexOf("_") + 1)}\` instead.`,
      ),
    ];
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "SPECIAL_AGENT_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const entries = Object.entries(env)
    .filter(
      ([key, value]) =>
        (key.startsWith("SPECIAL_AGENT_") || key.startsWith("SPECIAL_AGENT_")) && value?.trim(),
    )
    .map(([key]) => key);
  if (entries.length === 0) {
    return;
  ...
  const lines = [
    ...
    "- Use SPECIAL_AGENT_* equivalents instead:",
    ...entries.map((key) => {
      const suffix = key.slice(key.indexOf("_") + 1);
      return `  ${key} -> SPECIAL_AGENT_${suffix}`;
    }),
  ];
  (deps?.noteFn ?? note)(lines.join("\n"), "Environment");
}
