...
export function triggerSpecialAgentRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.SPECIAL_AGENT_SYSTEMD_UNIT,
        process.env.SPECIAL_AGENT_PROFILE,
      );
  ...
  const label =
    process.env.SPECIAL_AGENT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.SPECIAL_AGENT_PROFILE);
