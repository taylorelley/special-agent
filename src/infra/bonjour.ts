...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SPECIAL_AGENT_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "SpecialAgent";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(SpecialAgent\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.SPECIAL_AGENT_MDNS_HOSTNAME?.trim() ||
    process.env.SPECIAL_AGENT_MDNS_HOSTNAME?.trim() ||
    "special-agent";
  const hostname =
    hostnameRaw
