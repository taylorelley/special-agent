...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: SpecialAgentConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: SpecialAgentConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: SpecialAgentConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: SpecialAgentConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: SpecialAgentConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: SpecialAgentConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: SpecialAgentConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
