...
describe("device identity state dir defaults", () => {
  ...
  it("writes the default identity file under SPECIAL_AGENT_STATE_DIR", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-identity-state-"));
