...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "special-agent.mjs");
    ...
    if (!doctorEntryExists) {
      steps.push({
        name: "special-agent doctor entry",
    ...
    const doctorStep = await runStep(
      step("special-agent doctor", doctorArgv, gitRoot, { SPECIAL_AGENT_UPDATE_IN_PROGRESS: "1" }),
    );
