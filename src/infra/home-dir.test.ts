...
describe("resolveEffectiveHomeDir", () => {
  it("prefers SPECIAL_AGENT_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands SPECIAL_AGENT_HOME when set to ~", () => {
    const env = {
      SPECIAL_AGENT_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for SPECIAL_AGENT_HOME", () => {
    const result = resolveRequiredHomeDir(
      { SPECIAL_AGENT_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when SPECIAL_AGENT_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ SPECIAL_AGENT_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { SPECIAL_AGENT_HOME: "/srv/special-agent-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/special-agent-home")}/x`);
  });
