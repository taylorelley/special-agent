...
describe("resolve exec approvals defaults", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    const prevSpecialAgentHome = process.env.SPECIAL_AGENT_HOME;
    try {
      process.env.SPECIAL_AGENT_HOME = dir;
      expect(path.normalize(resolveExecApprovalsPath())).toBe(
        path.normalize(path.join(dir, ".special-agent", "exec-approvals.json")),
      );
      expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
        path.normalize(path.join(dir, ".special-agent", "exec-approvals.sock")),
      );
    } finally {
      if (prevSpecialAgentHome === undefined) {
        delete process.env.SPECIAL_AGENT_HOME;
      } else {
        process.env.SPECIAL_AGENT_HOME = prevSpecialAgentHome;
      }
    }
  });
});
...
describe("exec approvals shell allowlist (chained commands)", () => {
  ...
  it("respects quoted chain separators", () => {
    ...
    const commands = ['/usr/bin/echo "foo && bar"'];
