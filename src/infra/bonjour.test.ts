...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    process.env.SPECIAL_AGENT_MDNS_HOSTNAME = "test-host";
    process.env.SPECIAL_AGENT_MDNS_HOSTNAME = "test-host";
    ...
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.cliPath).toBe(
      "/opt/homebrew/bin/special-agent",
    );
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("special-agent (SpecialAgent)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("special-agent");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("special-agent.local");
