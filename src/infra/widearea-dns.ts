...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SPECIAL_AGENT_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*special-agent-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "special-agent";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "special-agent");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "special-agent-gw");
  ...
  return `; special-agent-content-hash: ${contentHash}\n${contentBody}`;
}
