...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-update-check-"));
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    const { resolveSpecialAgentPackageRoot } = await import("./special-agent-root.js");
    ...
    vi.mocked(resolveSpecialAgentPackageRoot).mockResolvedValue("/opt/special-agent");
    vi.mocked(checkUpdateStatus).mockResolvedValue({
      root: "/opt/special-agent",
      installKind: "package",
      packageManager: "npm",
