...
function candidateBinDirs(opts: EnsureSpecialAgentPathOpts): string[] {
  ...
  // Bundled macOS app: `special-agent` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "special-agent");
  ...
  if (isExecutable(path.join(localBinDir, "special-agent"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `special-agent` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureSpecialAgentCliOnPath(opts: EnsureSpecialAgentPathOpts = {}) {
  if (isTruthyEnvValue(process.env.SPECIAL_AGENT_PATH_BOOTSTRAPPED)) {
    return;
  }
