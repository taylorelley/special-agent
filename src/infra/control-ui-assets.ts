...
import { resolveSpecialAgentPackageRoot, resolveSpecialAgentPackageRootSync } from "./special-agent-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveSpecialAgentPackageRoot({ argv1: normalized });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "special-agent" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  if (argv1Dir) {
    // special-agent.mjs or dist/<bundle>.js
