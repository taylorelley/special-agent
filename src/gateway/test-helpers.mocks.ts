...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `special-agent-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.SPECIAL_AGENT_CONFIG_PATH = path.join(root, "special-agent.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "special-agent.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "special-agent-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadSpecialAgentPlugins: () => pluginRegistryState.registry,
  };
});
