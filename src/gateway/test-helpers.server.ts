...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.SPECIAL_AGENT_STATE_DIR;
  previousConfigPath = process.env.SPECIAL_AGENT_CONFIG_PATH;
  previousSkipBrowserControl = process.env.SPECIAL_AGENT_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.SPECIAL_AGENT_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.SPECIAL_AGENT_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.SPECIAL_AGENT_BUNDLED_PLUGINS_DIR;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.SPECIAL_AGENT_STATE_DIR = path.join(tempHome, ".special-agent");
  delete process.env.SPECIAL_AGENT_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.SPECIAL_AGENT_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.SPECIAL_AGENT_SKIP_GMAIL_WATCHER = "1";
  process.env.SPECIAL_AGENT_SKIP_CANVAS_HOST = "1";
  process.env.SPECIAL_AGENT_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "special-agent-test-no-bundled-extensions")
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "special-agent-test-"))
    : path.join(tempHome, ".special-agent-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.SPECIAL_AGENT_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.SPECIAL_AGENT_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.SPECIAL_AGENT_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.SPECIAL_AGENT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.SPECIAL_AGENT_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.SPECIAL_AGENT_SKIP_CANVAS_HOST;
    } else {
      process.env.SPECIAL_AGENT_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
...
export async function startServerWithClient(
  ...
  const prev = process.env.SPECIAL_AGENT_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.SPECIAL_AGENT_GATEWAY_TOKEN = fallbackToken;
  }
