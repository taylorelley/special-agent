...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/special-agent/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (SpecialAgent)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.SPECIAL_AGENT_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "special-agent");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.SPECIAL_AGENT_TAILNET_DNS?.trim();
