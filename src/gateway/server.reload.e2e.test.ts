...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.SPECIAL_AGENT_SKIP_CHANNELS;
    prevSkipGmail = process.env.SPECIAL_AGENT_SKIP_GMAIL_WATCHER;
    process.env.SPECIAL_AGENT_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.SPECIAL_AGENT_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.SPECIAL_AGENT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.SPECIAL_AGENT_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
