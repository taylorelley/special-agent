  ...
  const applyHotReload = async (
    ...
    if (plan.restartGmailWatcher) {
      ...
      if (!isTruthyEnvValue(process.env.SPECIAL_AGENT_SKIP_GMAIL_WATCHER)) {
        ...
        params.logHooks.info("skipping gmail watcher restart (SPECIAL_AGENT_SKIP_GMAIL_WATCHER=1)");
      }
    }
    ...
    if (plan.restartChannels.size > 0) {
      if (
        ...
        params.logChannels.info(
          "skipping channel reload (SPECIAL_AGENT_SKIP_CHANNELS=1 or SPECIAL_AGENT_SKIP_PROVIDERS=1)",
        );
      } else {
