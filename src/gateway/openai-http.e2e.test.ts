    ...
    try {
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "special-agent", messages: [{ role: "user", content: "hi" }] },
          { "x-special-agent-agent-id": "beta" },
        );
...

      {
        ...
        const res = await postChatCompletions(
          ...
          { "x-special-agent-agent-id": "alpha" },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const res = await postChatCompletions(
          ...
          {
            "x-special-agent-agent-id": "beta",
            "x-special-agent-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
