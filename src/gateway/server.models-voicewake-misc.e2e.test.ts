...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.SPECIAL_AGENT_STATE_DIR = path.join(homeDir, ".special-agent");
  ...
  test(
    ...
    { timeout: 60_000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-home-"));
      const restoreHome = setTempHome(homeDir);

      const initial = await rpcReq<{ triggers: string[] }>(ws, "voicewake.get");
      expect(initial.ok).toBe(true);
      expect(initial.payload?.triggers).toEqual(["special-agent", "claude", "computer"]);
      ...
      const onDisk = JSON.parse(
        await fs.readFile(path.join(homeDir, ".special-agent", "settings", "voicewake.json"), "utf8"),
  ...
  test("pushes voicewake.changed to nodes on connect and on updates", async () => {
    ...
    const setRes = await rpcReq<{ triggers: string[] }>(ws, "voicewake.set", {
      triggers: ["special-agent", "computer"],
    });
    expect(setRes.ok).toBe(true);
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    ...
    const prevCanvasPort = process.env.SPECIAL_AGENT_CANVAS_HOST_PORT;
    process.env.SPECIAL_AGENT_GATEWAY_TOKEN = "secret";
    testTailnetIPv4.value = "100.64.0.1";
    testState.gatewayBind = "lan";
    const canvasPort = await getFreePort();
    testState.canvasHostPort = canvasPort;
    process.env.SPECIAL_AGENT_CANVAS_HOST_PORT = String(canvasPort);
    ...
    if (prevCanvasPort === undefined) {
      ...
      process.env.SPECIAL_AGENT_CANVAS_HOST_PORT = prevCanvasPort;
    }
  });
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    const configPath = process.env.SPECIAL_AGENT_CONFIG_PATH;
    if (!configPath) {
      throw new Error("Missing SPECIAL_AGENT_CONFIG_PATH");
    }
