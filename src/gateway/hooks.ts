...
export function resolveHooksConfig(cfg: SpecialAgentConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: SpecialAgentConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-special-agent-token"] === "string"
      ? req.headers["x-special-agent-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
