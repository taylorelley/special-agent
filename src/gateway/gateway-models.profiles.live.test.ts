...
import type { SpecialAgentConfig, ModelProviderConfig } from "../config/types.js";
...
function sanitizeAuthConfig(params: {
  cfg: SpecialAgentConfig;
  agentDir: string;
}): SpecialAgentConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<SpecialAgentConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const hostAgentDir = resolveSpecialAgentAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-live-state-"));
  ...
  process.env.SPECIAL_AGENT_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.special-agent-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveSpecialAgentAgentDir();
  const sanitizedCfg: SpecialAgentConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-live-"));
  ...
  await ensureSpecialAgentModelsJson(nextCfg);
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "SpecialAgent live tool probe (local, safe): " +
    ...
    process.env.SPECIAL_AGENT_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.SPECIAL_AGENT_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.SPECIAL_AGENT_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.special-agent-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
