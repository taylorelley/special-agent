...
export async function startGatewayServer(
  ...
  process.env.SPECIAL_AGENT_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "SPECIAL_AGENT_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "SPECIAL_AGENT_RAW_STREAM_PATH",
  ...
  if (configSnapshot.legacyIssues.length > 0) {
    ...
    if (!migrated) {
      throw new Error(
        `Legacy config entries detected but auto-migration failed. Run "${formatCliCommand("special-agent doctor")}" to migrate.`,
      );
    }
  ...
  if (configSnapshot.exists && !configSnapshot.valid) {
    ...
    throw new Error(
      `Invalid config at ${configSnapshot.path}.\n${issues}\nRun "${formatCliCommand("special-agent doctor")}" to repair, then retry.`,
    );
  }
