...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.SPECIAL_AGENT_CONFIG_PATH;
  const prevDisableCache = process.env.SPECIAL_AGENT_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "special-agent-canvas-auth-test-"));
  const configPath = path.join(dir, "special-agent.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.SPECIAL_AGENT_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.SPECIAL_AGENT_DISABLE_CONFIG_CACHE;
    } else {
      process.env.SPECIAL_AGENT_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
