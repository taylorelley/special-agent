...
export function isCommandEnabled(cfg: SpecialAgentConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: SpecialAgentConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: SpecialAgentConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: SpecialAgentConfig) {
