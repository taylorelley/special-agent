...
describe("runReplyAgent typing (heartbeat)", () => {
  it("resets corrupted Gemini sessions and deletes transcripts", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "special-agent-session-reset-"));
  ...
  it("keeps sessions intact on other errors", async () => {
    const prevStateDir = process.env.SPECIAL_AGENT_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "special-agent-session-noreset-"));
    process.env.SPECIAL_AGENT_STATE_DIR = stateDir;
    try {
