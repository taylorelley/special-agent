...
import { scheduleGatewaySigusr1Restart, triggerSpecialAgentRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting SpecialAgent in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerSpecialAgentRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting SpecialAgent via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
