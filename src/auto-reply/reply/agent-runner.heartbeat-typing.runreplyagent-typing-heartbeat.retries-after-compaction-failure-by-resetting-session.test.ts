...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  it("retries after context overflow payload by resetting the session", async () => {
    const prevStateDir = process.env.SPECIAL_AGENT_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "special-agent-session-overflow-reset-"));
    process.env.SPECIAL_AGENT_STATE_DIR = stateDir;
    try {
      const sessionId = "session";
      const storePath = path.join(stateDir, "sessions", "sessions.json");
  ...
  it("resets the session after role ordering payloads", async () => {
    const prevStateDir = process.env.SPECIAL_AGENT_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "special-agent-session-role-ordering-"));
    process.env.SPECIAL_AGENT_STATE_DIR = stateDir;
    try {
      const sessionId = "session";
      const storePath = path.join(stateDir, "sessions", "sessions.json");
