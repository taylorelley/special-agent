...
type AgentConfig = Partial<NonNullable<NonNullable<SpecialAgentConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.special-agent/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `✈️ SpecialAgent ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: SpecialAgentConfig): string {
  const lines = ["ℹ️ Help", ""];
