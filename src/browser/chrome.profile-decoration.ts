...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".special-agent-profile-decorated");
}
 ...
 */
export function decorateSpecialAgentProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_SPECIAL_AGENT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SPECIAL_AGENT_BROWSER_COLOR).toUpperCase();
