...
describe("browser config", () => {
  ...
  it("derives default ports from SPECIAL_AGENT_GATEWAY_PORT when unset", () => {
    const prev = process.env.SPECIAL_AGENT_GATEWAY_PORT;
    process.env.SPECIAL_AGENT_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      if (prev === undefined) {
        ...
        process.env.SPECIAL_AGENT_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "special-agent");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    ...
    expect(resolved.defaultProfile).toBe("special-agent");
  });
});
