...
const RELAY_AUTH_HEADER = "x-special-agent-relay-token";
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/SpecialAgent-Extension-Relay",
          revision: "0",
          userAgent: "SpecialAgent-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "SpecialAgent/extension-relay",
