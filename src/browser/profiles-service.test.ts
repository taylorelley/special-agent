...
vi.mock("./chrome.js", () => ({
  resolveSpecialAgentUserDataDir: vi.fn(() => "/tmp/special-agent-test/special-agent/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveSpecialAgentUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "special-agent-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveSpecialAgentUserDataDir).mockReturnValue(userDataDir);
