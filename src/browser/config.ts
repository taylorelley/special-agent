import type { BrowserConfig, BrowserProfileConfig, SpecialAgentConfig } from "../config/config.js";
...
import {
  DEFAULT_SPECIAL_AGENT_BROWSER_COLOR,
  DEFAULT_SPECIAL_AGENT_BROWSER_ENABLED,
  DEFAULT_BROWSER_EVALUATE_ENABLED,
  DEFAULT_BROWSER_DEFAULT_PROFILE_NAME,
  DEFAULT_SPECIAL_AGENT_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "special-agent" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_SPECIAL_AGENT_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "special-agent" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_SPECIAL_AGENT_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_SPECIAL_AGENT_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an SpecialAgent browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for special-agent/special-agent CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: SpecialAgentConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_SPECIAL_AGENT_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "special-agent";
