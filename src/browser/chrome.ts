...
export function resolveSpecialAgentUserDataDir(profileName = DEFAULT_SPECIAL_AGENT_BROWSER_PROFILE_NAME) {
...
export async function launchSpecialAgentChrome(
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_SPECIAL_AGENT_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateSpecialAgentProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
  ...
  try {
    ...
    log.warn(`special-agent browser clean-exit prefs failed: ${String(err)}`);
  }
...
export async function stopSpecialAgentChrome(running: RunningChrome, timeoutMs = 2500) {
