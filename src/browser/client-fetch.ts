...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  const hint = isAbsoluteHttp(url)
    ...
    : `Start (or restart) the SpecialAgent gateway (SpecialAgent.app menubar, or \`${formatCliCommand("special-agent gateway")}\`) and try again.`;
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the SpecialAgent browser control service (timed out after ${timeoutMs}ms). ${hint}`,
    );
  }
  return new Error(`Can't reach the SpecialAgent browser control service. ${hint} (${msg})`);
}
