...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `special-agent --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.SPECIAL_AGENT_GATEWAY_PORT ??
    process.env.SPECIAL_AGENT_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.SPECIAL_AGENT_GATEWAY_BIND ??
    process.env.SPECIAL_AGENT_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
...
void main().catch((err) => {
  console.error(
    "[special-agent] Gateway daemon failed:",
