...
declare const __SPECIAL_AGENT_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __SPECIAL_AGENT_VERSION__ === "string" && __SPECIAL_AGENT_VERSION__) ||
  process.env.SPECIAL_AGENT_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  ...
  const { ensureSpecialAgentCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[special-agent] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
