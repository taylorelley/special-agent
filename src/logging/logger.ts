...
export const DEFAULT_LOG_DIR = "/tmp/special-agent";
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "special-agent.log"); // legacy single-file path

const LOG_PREFIX = "special-agent";
...
function resolveSettings(): ResolvedSettings {
  let cfg: SpecialAgentConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => SpecialAgentConfig;
      };
