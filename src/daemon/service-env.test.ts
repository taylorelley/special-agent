...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.SPECIAL_AGENT_GATEWAY_PORT).toBe("18789");
    expect(env.SPECIAL_AGENT_GATEWAY_TOKEN).toBe("secret");
    expect(env.SPECIAL_AGENT_SERVICE_MARKER).toBe("special-agent");
    expect(env.SPECIAL_AGENT_SERVICE_KIND).toBe("gateway");
    expect(typeof env.SPECIAL_AGENT_SERVICE_VERSION).toBe("string");
    expect(env.SPECIAL_AGENT_SYSTEMD_UNIT).toBe("special-agent-gateway.service");
    if (process.platform === "darwin") {
      expect(env.SPECIAL_AGENT_LAUNCHD_LABEL).toBe("ai.special-agent.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", SPECIAL_AGENT_PROFILE: "work" },
      port: 18789,
    });
    expect(env.SPECIAL_AGENT_SYSTEMD_UNIT).toBe("special-agent-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.SPECIAL_AGENT_LAUNCHD_LABEL).toBe("ai.special-agent.work");
    }
  });
});
