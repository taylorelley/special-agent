...
beforeEach(async () => {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-web-home-"));
...
const makeSessionStore = async (
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-session-"));
...
describe("web auto-reply", () => {
  ...
  it("ignores JID mentions in self-chat mode (group chats)", async () => {
    ...
    setLoadConfigMock(() => ({
      ...
      messages: {
        groupChat: {
          mentionPatterns: ["\\bspecial-agent\\b"],
        },
      },
    }));
    ...
    // Text-based mentionPatterns still work (user can type "special-agent" explicitly).
    await capturedOnMessage?.({
      body: "special-agent ping",
      from: "123@g.us",
      conversationId: "123@g.us",
      chatId: "123@g.us",
      chatType: "group",
  ...
  it("emits heartbeat logs with connection metadata", async () => {
    vi.useFakeTimers();
    const logPath = `/tmp/special-agent-heartbeat-${crypto.randomUUID()}.log`;
  ...
  it("logs outbound replies to file", async () => {
    const logPath = `/tmp/special-agent-log-test-${crypto.randomUUID()}.log`;
    setLoggerOverride({ level: "trace", file: logPath });
