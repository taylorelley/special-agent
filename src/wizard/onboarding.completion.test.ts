...
describe("setupOnboardingShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    const deps = {
      resolveCliName: () => "special-agent",
      checkShellCompletionStatus: vi.fn(async () => ({
        shell: "zsh",
        profileInstalled: false,
        cacheExists: false,
        cachePath: "/tmp/special-agent.zsh",
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("special-agent");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "special-agent");
