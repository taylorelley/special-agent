...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    ...
    "When diagnosing issues, run `special-agent status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running SpecialAgent process",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside SpecialAgent.";
  }

  const lines = [
    "You are a personal assistant running inside SpecialAgent.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control SpecialAgent's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    "",
    ...safetySection,
    "## SpecialAgent CLI Quick Reference",
    "SpecialAgent is controlled via subcommands. Do not invent commands.",
    ...
    "If unsure, ask the user to run `special-agent help` (or `special-agent gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    ...
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, SpecialAgent pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by SpecialAgent and included below in Project Context.",
    "",
  ...
  if (!isMinimal) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'SpecialAgent treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
