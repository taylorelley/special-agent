...
const _MODELS_CONFIG: SpecialAgentConfig = {
  models: {
    providers: {
      "custom-proxy": {
  ...
  it("auto-injects github-copilot provider when token is present", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        ...
        await ensureSpecialAgentModelsJson({ models: { providers: {} } }, agentDir);
  ...
  it("prefers COPILOT_GITHUB_TOKEN over GH_TOKEN and GITHUB_TOKEN", async () => {
    await withTempHome(async () => {
      ...
      try {
        ...
        await ensureSpecialAgentModelsJson({ models: { providers: {} } });
