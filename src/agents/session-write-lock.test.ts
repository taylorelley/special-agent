...
describe("acquireSessionWriteLock", () => {
  it("reuses locks across symlinked session paths", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-lock-"));
    try {
  ...
  it("removes held locks on termination signals", async () => {
    ...
    for (const signal of signals) {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-lock-cleanup-"));
      try {
        const sessionFile = path.join(root, "sessions.json");
        const lockPath = `${sessionFile}.lock`;
        await acquireSessionWriteLock({ sessionFile, timeoutMs: 500 });
        // Prevent handleTerminationSignal from re-raising the signal
        // (which would kill the worker process).
        const keepAlive = () => {};
