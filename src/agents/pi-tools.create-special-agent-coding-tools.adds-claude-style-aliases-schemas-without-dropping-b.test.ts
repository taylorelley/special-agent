...
describe("createSpecialAgentCodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    const config: SpecialAgentConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createSpecialAgentCodingTools({
      config,
    ...
    const anthropicTools = createSpecialAgentCodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createSpecialAgentCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = createSpecialAgentCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
