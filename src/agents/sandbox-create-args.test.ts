...
describe("buildSandboxCreateArgs", () => {
  it("includes hardening and resource flags", () => {
    ...
    expect(args).toEqual(
      expect.arrayContaining([
        "create",
        "--name",
        "special-agent-sbx-test",
        "--label",
        "special-agent.sandbox=1",
        "--label",
        "special-agent.sessionKey=main",
        "--label",
        "special-agent.createdAtMs=1700000000000",
        "--label",
        "special-agent.sandboxBrowser=1",
        "--read-only",
        "--tmpfs",
        "/tmp",
        "--network",
        ...
        "--security-opt",
        "apparmor=special-agent-sandbox",
        "--dns",
        "1.1.1.1",
        "--add-host",
  ...
  it("omits -v flags when binds is empty or undefined", () => {
    ...
    const args = buildSandboxCreateArgs({
      name: "special-agent-sbx-no-binds",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
    });
