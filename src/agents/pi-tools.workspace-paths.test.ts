...
import { createSpecialAgentCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("special-agent-ws-", async (workspaceDir) => {
      await withTempDir("special-agent-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createSpecialAgentCodingTools({ workspaceDir });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("special-agent-ws-", async (workspaceDir) => {
      const tools = createSpecialAgentCodingTools({ workspaceDir, exec: { host: "gateway" } });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("special-agent-ws-", async (workspaceDir) => {
      await withTempDir("special-agent-override-", async (overrideDir) => {
        const tools = createSpecialAgentCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("special-agent-sandbox-", async (sandboxDir) => {
      await withTempDir("special-agent-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "special-agent-sbx-test",
        ...
        const tools = createSpecialAgentCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
