...
import { __testing, createSpecialAgentCodingTools } from "./pi-tools.js";
...
describe("createSpecialAgentCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createSpecialAgentTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createSpecialAgentCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "special-agent-outside.txt");
