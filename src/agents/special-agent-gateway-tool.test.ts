...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const previousProfile = process.env.SPECIAL_AGENT_PROFILE;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-test-"));
    process.env.SPECIAL_AGENT_STATE_DIR = stateDir;
    process.env.SPECIAL_AGENT_PROFILE = "isolated";

    try {
      const tool = createSpecialAgentTools({
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: special-agent --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/special-agent" } }\n}\n';
