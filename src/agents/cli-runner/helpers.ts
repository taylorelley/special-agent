...
/**
 * Cleanup suspended SpecialAgent CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: SpecialAgentConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-cli-images-"));
  const paths: string[] = [];
