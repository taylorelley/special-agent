...
describe("sandbox fs bridge shell compatibility", () => {
  ...
  it("resolves bind-mounted absolute container paths for reads", async () => {
    ...
    expect(args).toEqual(
      expect.arrayContaining(["special-agent-sbx-test", "sh", "-c", 'set -eu; cat -- "$1"']),
    );
  ...
  it("allows mkdirp for existing in-boundary subdirectories", async () => {
    await withTempDir("special-agent-fs-bridge-mkdirp-", async (stateDir) => {
  ...
  it("rejects mkdirp when target exists as a file", async () => {
    await withTempDir("special-agent-fs-bridge-mkdirp-file-", async (stateDir) => {
      const workspaceDir = path.join(stateDir, "workspace");
  ...
  it("rejects pre-existing host symlink escapes before docker exec", async () => {
    await withTempDir("special-agent-fs-bridge-", async (stateDir) => {
      const workspaceDir = path.join(stateDir, "workspace");
  ...
  it("rejects pre-existing host hardlink escapes before docker exec", async () => {
    ...
    await withTempDir("special-agent-fs-bridge-hardlink-", async (stateDir) => {
      const workspaceDir = path.join(stateDir, "workspace");
      const outsideDir = path.join(stateDir, "outside");
      const outsideFile = path.join(outsideDir, "secret.txt");
      await fs.mkdir(workspaceDir, { recursive: true });
