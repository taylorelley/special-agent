...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", `special-agent.sessionKey=${params.scopeKey}`);
  args.push("--label", `special-agent.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `special-agent.configHash=${params.configHash}`);
  }
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`special-agent sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`special-agent sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("special-agent sandbox recreate --all");
}
