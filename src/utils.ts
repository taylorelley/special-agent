...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".special-agent");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.SPECIAL_AGENT_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$SPECIAL_AGENT_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via SPECIAL_AGENT_STATE_DIR.
