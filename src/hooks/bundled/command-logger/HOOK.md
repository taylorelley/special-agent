...
homepage: https://docs.special-agent.ai/hooks#command-logger
metadata:
  {
    "special-agent":
      {
        "emoji": "📝",
        "events": ["command"],
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Special Agent" }],
      },
  }
---
...
2. **Appends to log file** - Writes a JSON line to `~/.special-agent/logs/commands.log`
...
`~/.special-agent/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.special-agent/logs/commands.log ~/.special-agent/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/special-agent`:
   ```
   /home/username/.special-agent/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
