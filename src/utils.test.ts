...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "special-agent-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "special-agent-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "special-agent-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "special-agent-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.special-agent when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "special-agent-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers SPECIAL_AGENT_HOME over HOME", () => {
    vi.stubEnv("SPECIAL_AGENT_HOME", "/srv/special-agent-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/special-agent-home"));
...
describe("shortenHomePath", () => {
  it("uses $SPECIAL_AGENT_HOME prefix when SPECIAL_AGENT_HOME is set", () => {
    vi.stubEnv("SPECIAL_AGENT_HOME", "/srv/special-agent-home");
    vi.stubEnv("HOME", "/home/other");
...
describe("shortenHomeInString", () => {
  it("uses $SPECIAL_AGENT_HOME replacement when SPECIAL_AGENT_HOME is set", () => {
    vi.stubEnv("SPECIAL_AGENT_HOME", "/srv/special-agent-home");
    vi.stubEnv("HOME", "/home/other");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/special-agent")).toBe(path.resolve(os.homedir(), "special-agent"));
  });
  ...
  it("prefers SPECIAL_AGENT_HOME for tilde expansion", () => {
    vi.stubEnv("SPECIAL_AGENT_HOME", "/srv/special-agent-home");
    vi.stubEnv("HOME", "/home/other");
