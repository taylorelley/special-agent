...
import { loadSpecialAgentPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.SPECIAL_AGENT_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `special-agent-plugin-${randomUUID()}`);
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.SPECIAL_AGENT_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadSpecialAgentPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.SPECIAL_AGENT_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadSpecialAgentPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadSpecialAgentPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
