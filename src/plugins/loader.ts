...
import { discoverSpecialAgentPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: SpecialAgentPluginDefinition;
  register?: SpecialAgentPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as SpecialAgentPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as SpecialAgentPluginDefinition;
...
export function loadSpecialAgentPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverSpecialAgentPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "special-agent/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: SpecialAgentPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as SpecialAgentPluginModule;
    } catch (err) {
