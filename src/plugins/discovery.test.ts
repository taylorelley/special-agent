...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `special-agent-plugins-${randomUUID()}`);
...
async function withStateDir<T>(stateDir: string, fn: () => Promise<T>) {
  const prev = process.env.SPECIAL_AGENT_STATE_DIR;
  const prevBundled = process.env.SPECIAL_AGENT_BUNDLED_PLUGINS_DIR;
  process.env.SPECIAL_AGENT_STATE_DIR = stateDir;
  process.env.SPECIAL_AGENT_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  vi.resetModules();
  try {
    return await fn();
  } finally {
    if (prev === undefined) {
      ...
      process.env.SPECIAL_AGENT_STATE_DIR = prev;
    }
    if (prevBundled === undefined) {
      delete process.env.SPECIAL_AGENT_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.SPECIAL_AGENT_BUNDLED_PLUGINS_DIR = prevBundled;
    }
    vi.resetModules();
  }
}
...
describe("discoverSpecialAgentPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".special-agent", "extensions");
    fs.mkdirSync(workspaceExt, { recursive: true });
    fs.writeFileSync(path.join(workspaceExt, "beta.ts"), "export default function () {}", "utf-8");

    const { candidates } = await withStateDir(stateDir, async () => {
      const { discoverSpecialAgentPlugins } = await import("./discovery.js");
      return discoverSpecialAgentPlugins({ workspaceDir });
    });
  ...
  it("loads package extension packs", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      const { discoverSpecialAgentPlugins } = await import("./discovery.js");
      return discoverSpecialAgentPlugins({});
    });
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      const { discoverSpecialAgentPlugins } = await import("./discovery.js");
      return discoverSpecialAgentPlugins({ extraPaths: [packDir] });
    });
