...
function setSlackDmPolicy(cfg: SpecialAgentConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "SpecialAgent";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for SpecialAgent`,
    },
    features: {
      bot_user: {
      ...
      slash_commands: [
        {
          ...
          description: "Send a message to SpecialAgent",
          should_escape: false,
        },
      ],
    },
...
function setSlackAllowFrom(cfg: SpecialAgentConfig, allowFrom: string[]): SpecialAgentConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
