...
function setTelegramDmPolicy(cfg: SpecialAgentConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
async function noteTelegramTokenHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Website: https://special-agent.ai",
    ].join("\n"),
    "Telegram bot token",
  );
}
...
async function noteTelegramUserIdHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      `1) DM your bot, then read from.id in \`${formatCliCommand("special-agent logs --follow")}\` (safest)`,
