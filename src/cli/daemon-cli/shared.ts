...
const SAFE_DAEMON_ENV_KEYS = [
  "SPECIAL_AGENT_PROFILE",
  "SPECIAL_AGENT_STATE_DIR",
  "SPECIAL_AGENT_CONFIG_PATH",
  "SPECIAL_AGENT_GATEWAY_PORT",
  "SPECIAL_AGENT_NIX_MODE",
];
...
export function renderRuntimeHints(
  ...
  if (runtime.status === "stopped") {
    ...
    if (process.platform === "darwin") {
      ...
      const task = resolveGatewayWindowsTaskName(env.SPECIAL_AGENT_PROFILE);
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  const base = [
    formatCliCommand("special-agent gateway install", env),
    formatCliCommand("special-agent gateway", env),
  ];
