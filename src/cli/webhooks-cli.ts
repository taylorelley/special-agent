...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.special-agent.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + SpecialAgent hooks")
    ...
    .option("--hook-url <url>", "SpecialAgent hook URL")
    .option("--hook-token <token>", "SpecialAgent hook token")
