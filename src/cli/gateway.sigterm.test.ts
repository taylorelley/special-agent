...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "special-agent-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      SPECIAL_AGENT_NO_RESPAWN: "1",
      SPECIAL_AGENT_STATE_DIR: stateDir,
      SPECIAL_AGENT_SKIP_CHANNELS: "1",
      SPECIAL_AGENT_SKIP_GMAIL_WATCHER: "1",
      SPECIAL_AGENT_SKIP_CRON: "1",
      SPECIAL_AGENT_SKIP_BROWSER_CONTROL_SERVER: "1",
      SPECIAL_AGENT_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "special-agent-entry-bootstrap.mjs");
