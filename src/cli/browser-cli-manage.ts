...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "special-agent"}`,
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        const name = status.profile ?? "special-agent";
        defaultRuntime.log(info(`✈️ browser [${name}] running: ${status.running}`));
      });
    });
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        if (!result.moved) {
          defaultRuntime.log(info(`✈️ browser profile already missing.`));
          return;
        }
        const dest = result.to ?? result.from;
        defaultRuntime.log(info(`✈️ browser profile moved to Trash (${dest})`));
      });
    });
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (special-agent|extension). Default: special-agent")
    .action(
      async (opts: { name: string; color?: string; cdpUrl?: string; driver?: string }, cmd) => {
        ...
        await runBrowserCommand(async () => {
          ...
          defaultRuntime.log(
            info(
              `✈️ Created profile "${result.profile}"\n${loc}\n  color: ${result.color}${
    ...
    .action(async (opts: { name: string }, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        const msg = result.deleted
          ? `✈️ Deleted profile "${result.profile}" (user data removed)`
          : `✈️ Deleted profile "${result.profile}" (no user data found)`;
