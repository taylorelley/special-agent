...
export function registerDnsCli(program: Command) {
  ...
  dns
    ...
    .option("--domain <domain>", "Wide-area discovery domain (e.g. special-agent.internal)")
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log("");
      defaultRuntime.log(theme.heading("Recommended ~/.special-agent/special-agent.json:"));
      defaultRuntime.log(
        JSON.stringify(
          {
            gateway: { bind: "auto" },
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by special-agent dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in ~/.special-agent/special-agent.json on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
