...
export function applyCliProfileEnv(params: {
  ...
  if (!env.SPECIAL_AGENT_CONFIG_PATH?.trim()) {
    env.SPECIAL_AGENT_CONFIG_PATH = path.join(stateDir, "special-agent.json");
  }

  if (profile === "dev" && !env.SPECIAL_AGENT_GATEWAY_PORT?.trim()) {
    env.SPECIAL_AGENT_GATEWAY_PORT = "19001";
  }
}
