...
${formatHelpExamples([
  ['special-agent agent --to +15555550123 --message "status update"', "Start a new session."],
  ['special-agent agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'special-agent agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'special-agent agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['special-agent agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'special-agent agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.special-agent.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.special-agent.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "special-agent agents set-identity --identity-file ~/.special-agent/workspace/IDENTITY.md --agent main",
