...
const STEP_LABELS: Record<string, string> = {
  ...
  "special-agent doctor entry": "Checking doctor entrypoint",
  "special-agent doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "special-agent";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const SPECIAL_AGENT_REPO_URL = "https://github.com/special-agent/special-agent.git";
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("special-agent@")) {
    return trimmed.slice("special-agent@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "special-agent.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("special-agent completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
function resolveGitInstallDir(): string {
  const override = process.env.SPECIAL_AGENT_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", SPECIAL_AGENT_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `SPECIAL_AGENT_GIT_DIR points at a non-git directory: ${params.dir}. Set SPECIAL_AGENT_GIT_DIR to an empty folder or an special-agent checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`SPECIAL_AGENT_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveSpecialAgentPackageRoot({
  ...
  defaultRuntime.log(theme.heading("SpecialAgent update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating SpecialAgent..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this SpecialAgent install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("special-agent doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("special-agent gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g special-agent@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g special-agent@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.SPECIAL_AGENT_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.SPECIAL_AGENT_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("special-agent gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("special-agent doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("special-agent gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("special-agent gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `special-agent update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `SPECIAL_AGENT_GIT_DIR points at a non-git directory: ${gitDir}. Set SPECIAL_AGENT_GIT_DIR to an empty folder or an special-agent checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via SPECIAL_AGENT_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update SpecialAgent to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["special-agent update", "Update a source checkout (git)"],
        ["special-agent update --channel beta", "Switch to beta channel (git + npm)"],
        ["special-agent update --channel dev", "Switch to dev channel (git + npm)"],
        ["special-agent update --tag beta", "One-off update to a dist-tag or version"],
        ["special-agent update --no-restart", "Update without restarting the service"],
        ["special-agent update --json", "Output result as JSON"],
        ["special-agent update --yes", "Non-interactive (accept downgrade prompts)"],
        ["special-agent update wizard", "Interactive update wizard"],
        ["special-agent --update", "Shorthand for special-agent update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run special-agent update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.special-agent.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["special-agent update status", "Show channel + version status."],
          ["special-agent update status --json", "JSON output."],
          ["special-agent update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.special-agent.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
