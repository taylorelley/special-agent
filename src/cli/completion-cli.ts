...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# SpecialAgent Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(special-agent completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# SpecialAgent Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(special-agent completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $SPECIAL_AGENT_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "special-agent") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c special-agent -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'special-agent gateway' and not 'special-agent other gateway', we need to check the sequence.
