...
<title>SpecialAgent Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.specialAgentCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.specialAgentCanvasA2UIAction &&
        typeof window.specialAgentCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.specialAgentSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("special-agent:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android SpecialAgent node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.specialAgentSendUserAction === "function"
        ? window.specialAgentSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SPECIAL_AGENT_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.SPECIAL_AGENT_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>SpecialAgent Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
