# Special Agent ACP Bridge

This document describes how the Special Agent ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`special-agent acp` exposes an ACP agent over stdio and forwards prompts to a running
Special Agent Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Special Agent Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `special-agent acp` over stdio.

Example config:

```bash
special-agent config set gateway.remote.url wss://gateway-host:18789
special-agent config set gateway.remote.token <token>
```

Example run:

```bash
special-agent acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
special-agent acp --session agent:main:main
special-agent acp --session agent:design:main
special-agent acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Special Agent ACP” to start a thread.

## Execution Model

- ACP client spawns `special-agent acp` and speaks ACP messages over stdio.
...
```bash
special-agent acp --session agent:main:main
special-agent acp --session-label "support inbox"
special-agent acp --reset-session
```
...
`special-agent acp` resolves the Gateway URL and auth from CLI flags or config:
