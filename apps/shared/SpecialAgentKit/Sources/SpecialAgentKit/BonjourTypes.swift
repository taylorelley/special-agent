...
public enum SpecialAgentBonjour {
    // v0: internal-only, subject to rename.
    public static let gatewayServiceType = "_special-agent-gw._tcp"
    public static let gatewayServiceDomain = "local."
    public static var wideAreaGatewayServiceDomain: String? {
        let env = ProcessInfo.processInfo.environment
        return resolveWideAreaDomain(env["SPECIAL_AGENT_WIDE_AREA_DOMAIN"])
    }
