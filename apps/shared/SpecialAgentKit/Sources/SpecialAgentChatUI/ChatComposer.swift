...
@MainActor
struct SpecialAgentChatComposer: View {
    @Bindable var viewModel: SpecialAgentChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(SpecialAgentChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(SpecialAgentChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \SpecialAgentPendingAttachment.id)
                { (att: SpecialAgentPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            SpecialAgentPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(SpecialAgentChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(SpecialAgentChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(SpecialAgentChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(SpecialAgentChatTheme.subtleCard)
        .clipShape(Capsule())
    }
