...
enum SpecialAgentChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("SpecialAgentChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("SpecialAgentChatTheme.onboardingAssistantBubble"),
...
enum SpecialAgentPlatformImageFactory {
    static func image(_ image: SpecialAgentPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
