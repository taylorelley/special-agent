...
struct ChatMessageBubble: View {
    let message: SpecialAgentChatMessage
    let style: SpecialAgentChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? SpecialAgentChatTheme.userText : SpecialAgentChatTheme.assistantText
    ...
    private var inlineAttachments: [SpecialAgentChatMessageContent] {
    ...
    private var toolCalls: [SpecialAgentChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [SpecialAgentChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? SpecialAgentChatTheme.userBubble
        }
        if self.style == .onboarding {
            return SpecialAgentChatTheme.onboardingAssistantBubble
        }
        return SpecialAgentChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return SpecialAgentChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: SpecialAgentChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? SpecialAgentChatTheme.userText : SpecialAgentChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: SpecialAgentChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(SpecialAgentChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(SpecialAgentChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [SpecialAgentChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: SpecialAgentChatTheme.assistantText)
            }
        }
    }
}
