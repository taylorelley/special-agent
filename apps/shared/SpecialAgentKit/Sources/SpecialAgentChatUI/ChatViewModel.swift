...
private let chatUILogger = Logger(subsystem: "ai.special-agent", category: "SpecialAgentChatUI")

@MainActor
@Observable
public final class SpecialAgentChatViewModel {
    public private(set) var messages: [SpecialAgentChatMessage] = []
    ...
    public var attachments: [SpecialAgentPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [SpecialAgentChatPendingToolCall] = []
    public private(set) var sessions: [SpecialAgentChatSessionEntry] = []
    private let transport: any SpecialAgentChatTransport
    ...
    private var pendingToolCallsById: [String: SpecialAgentChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any SpecialAgentChatTransport) {
    ...
    public var sessionChoices: [SpecialAgentChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [SpecialAgentChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [SpecialAgentChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: SpecialAgentPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [SpecialAgentChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: SpecialAgentChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [SpecialAgentChatMessage]) -> [SpecialAgentChatMessage] {
        var result: [SpecialAgentChatMessage] = []
    ...
    private static func dedupeKey(for message: SpecialAgentChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [SpecialAgentChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> SpecialAgentChatAttachmentPayload in
            SpecialAgentChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> SpecialAgentChatSessionEntry {
        SpecialAgentChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: SpecialAgentChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: SpecialAgentChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: SpecialAgentAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = SpecialAgentChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> SpecialAgentPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
