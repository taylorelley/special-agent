...
import Foundation
import Testing
@testable import SpecialAgentChatUI
...
private final class TestChatTransport: @unchecked Sendable, SpecialAgentChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [SpecialAgentChatHistoryPayload]
    private let sessionsResponses: [SpecialAgentChatSessionsListResponse]

    private let stream: AsyncStream<SpecialAgentChatTransportEvent>
    private let continuation: AsyncStream<SpecialAgentChatTransportEvent>.Continuation

    init(
        historyResponses: [SpecialAgentChatHistoryPayload],
        sessionsResponses: [SpecialAgentChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<SpecialAgentChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> SpecialAgentChatHistoryPayload {
        ...
        return self.historyResponses.last ?? SpecialAgentChatHistoryPayload(
        ...
        attachments _: [SpecialAgentChatAttachmentPayload]) async throws -> SpecialAgentChatSendResponse
    {
        ...
        return SpecialAgentChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> SpecialAgentChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? SpecialAgentChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: SpecialAgentChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = SpecialAgentChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = SpecialAgentChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { SpecialAgentChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = SpecialAgentChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = SpecialAgentChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = SpecialAgentChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { SpecialAgentChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
