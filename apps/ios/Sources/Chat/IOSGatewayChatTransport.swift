...
struct IOSGatewayChatTransport: SpecialAgentChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> SpecialAgentChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(SpecialAgentChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> SpecialAgentChatHistoryPayload {
        ...
        return try JSONDecoder().decode(SpecialAgentChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [SpecialAgentChatAttachmentPayload]) async throws -> SpecialAgentChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [SpecialAgentChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(SpecialAgentChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(SpecialAgentGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SpecialAgentGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SpecialAgentChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SpecialAgentAgentEventPayload.self)
                        {
