...
final class ContactsService: ContactsServicing {
    ...
    func search(params: SpecialAgentContactsSearchParams) async throws -> SpecialAgentContactsSearchPayload {
        ...
        return SpecialAgentContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: SpecialAgentContactsAddParams) async throws -> SpecialAgentContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return SpecialAgentContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return SpecialAgentContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> SpecialAgentContactPayload {
        SpecialAgentContactPayload(
