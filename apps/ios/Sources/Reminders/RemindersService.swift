...
final class RemindersService: RemindersServicing {
    func list(params: SpecialAgentRemindersListParams) async throws -> SpecialAgentRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[SpecialAgentReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return SpecialAgentReminderPayload(
        ...
        return SpecialAgentRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: SpecialAgentRemindersAddParams) async throws -> SpecialAgentRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = SpecialAgentReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return SpecialAgentRemindersAddPayload(reminder: payload)
    }
