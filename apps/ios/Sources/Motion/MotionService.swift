...
final class MotionService: MotionServicing {
    func activities(params: SpecialAgentMotionActivityParams) async throws -> SpecialAgentMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[SpecialAgentMotionActivityEntry], Error>) in
            manager.queryActivityStarting(from: start, to: end, to: OperationQueue()) { activity, error in
                if let error {
                    ...
                    let entries = sliced.map { entry in
                        SpecialAgentMotionActivityEntry(
        ...
        return SpecialAgentMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: SpecialAgentPedometerParams) async throws -> SpecialAgentPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<SpecialAgentPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = SpecialAgentPedometerPayload(
