...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> SpecialAgentNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> SpecialAgentNetworkStatusPayload {
        let status: SpecialAgentNetworkPathStatus = switch path.status {
        ...
        var interfaces: [SpecialAgentNetworkInterfaceType] = []
        if path.usesInterfaceType(.wifi) { interfaces.append(.wifi) }
        if path.usesInterfaceType(.cellular) { interfaces.append(.cellular) }
        if path.usesInterfaceType(.wiredEthernet) { interfaces.append(.wired) }
        if interfaces.isEmpty { interfaces.append(.other) }

        return SpecialAgentNetworkStatusPayload(
            status: status,
    ...
    private static func fallbackPayload() -> SpecialAgentNetworkStatusPayload {
        SpecialAgentNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
