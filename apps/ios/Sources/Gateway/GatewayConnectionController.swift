...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [SpecialAgentCapability.canvas.rawValue, SpecialAgentCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(SpecialAgentCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(SpecialAgentCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = SpecialAgentLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(SpecialAgentCapability.location.rawValue) }

        caps.append(SpecialAgentCapability.device.rawValue)
        caps.append(SpecialAgentCapability.photos.rawValue)
        caps.append(SpecialAgentCapability.contacts.rawValue)
        caps.append(SpecialAgentCapability.calendar.rawValue)
        caps.append(SpecialAgentCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(SpecialAgentCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            SpecialAgentSystemCommand.notify.rawValue,
            SpecialAgentChatCommand.push.rawValue,
        ...
        if caps.contains(SpecialAgentCapability.camera.rawValue) {
            commands.append(SpecialAgentCameraCommand.list.rawValue)
            commands.append(SpecialAgentCameraCommand.snap.rawValue)
            commands.append(SpecialAgentCameraCommand.clip.rawValue)
        }
        if caps.contains(SpecialAgentCapability.location.rawValue) {
            commands.append(SpecialAgentLocationCommand.get.rawValue)
        }
        if caps.contains(SpecialAgentCapability.device.rawValue) {
            commands.append(SpecialAgentDeviceCommand.status.rawValue)
            commands.append(SpecialAgentDeviceCommand.info.rawValue)
        }
        if caps.contains(SpecialAgentCapability.photos.rawValue) {
            commands.append(SpecialAgentPhotosCommand.latest.rawValue)
        }
        if caps.contains(SpecialAgentCapability.contacts.rawValue) {
            commands.append(SpecialAgentContactsCommand.search.rawValue)
            commands.append(SpecialAgentContactsCommand.add.rawValue)
        }
        if caps.contains(SpecialAgentCapability.calendar.rawValue) {
            commands.append(SpecialAgentCalendarCommand.events.rawValue)
            commands.append(SpecialAgentCalendarCommand.add.rawValue)
        }
        if caps.contains(SpecialAgentCapability.reminders.rawValue) {
            commands.append(SpecialAgentRemindersCommand.list.rawValue)
            commands.append(SpecialAgentRemindersCommand.add.rawValue)
        }
        if caps.contains(SpecialAgentCapability.motion.rawValue) {
            commands.append(SpecialAgentMotionCommand.activity.rawValue)
            commands.append(SpecialAgentMotionCommand.pedometer.rawValue)
        }
