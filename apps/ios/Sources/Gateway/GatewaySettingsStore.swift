...
enum GatewaySettingsStore {
    private static let gatewayService = "ai.special-agent.gateway"
    private static let nodeService = "ai.special-agent.node"
...
enum GatewayDiagnostics {
    private static let logger = Logger(subsystem: "ai.special-agent.ios", category: "GatewayDiag")
    private static let queue = DispatchQueue(label: "ai.special-agent.gateway.diagnostics")
    ...
    private static var fileURL: URL? {
        FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first?
            .appendingPathComponent("special-agent-gateway.log")
    }
