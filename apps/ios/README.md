...
This is an **alpha** iOS app that connects to an Special Agent Gateway as a `role: node`.
...
3. On iOS: Special Agent → Settings → Gateway → paste setup code → Connect
...
1. Select the `Special Agent` scheme
...
cd apps/ios
xcodegen generate
xcodebuild test -project SpecialAgent.xcodeproj -scheme Special Agent -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/SpecialAgentKit` contains the shared transport/types used by the iOS app.
