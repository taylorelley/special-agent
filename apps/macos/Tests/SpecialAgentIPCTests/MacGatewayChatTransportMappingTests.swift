...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: SpecialAgentProtocol.AnyCodable(["ok": SpecialAgentProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: SpecialAgentProtocol.AnyCodable(["ok": SpecialAgentProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = SpecialAgentProtocol.AnyCodable([
            "runId": SpecialAgentProtocol.AnyCodable("run-1"),
            "sessionKey": SpecialAgentProtocol.AnyCodable("main"),
            "state": SpecialAgentProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: SpecialAgentProtocol.AnyCodable(["a": SpecialAgentProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
