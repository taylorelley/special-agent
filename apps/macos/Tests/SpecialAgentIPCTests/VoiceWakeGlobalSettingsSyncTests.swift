...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    @Test func appliesVoiceWakeChangedEventToAppState() async {
        ...
        let payload = SpecialAgentProtocol.AnyCodable(["triggers": ["special-agent", "computer"]])
        let evt = EventFrame(
            type: "event",
        ...
        #expect(updated == ["special-agent", "computer"])
    ...
    @Test func ignoresVoiceWakeChangedEventWithInvalidPayload() async {
        ...
        let payload = SpecialAgentProtocol.AnyCodable(["unexpected": 123])
        let evt = EventFrame(
            type: "event",
            event: "voicewake.changed",
            payload: payload,
