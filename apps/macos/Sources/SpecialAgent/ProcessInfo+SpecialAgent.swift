...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.special-agent.mac`) even if the shipped
    ...
    static func resolveNixMode(
        ...
        if environment["SPECIAL_AGENT_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "special-agent.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "special-agent.nixMode") { return true }
