...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.special-agent/special-agent.json. Prefer SPECIAL_AGENT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.special-agent/special-agent.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.special-agent/special-agent.json. Restart the gateway to apply."
        }
