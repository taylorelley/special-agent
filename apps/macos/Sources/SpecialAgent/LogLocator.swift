...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["SPECIAL_AGENT_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/special-agent")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("special-agent-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("special-agent-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/special-agent/ (rolling or stdout), or nil if none exist.
