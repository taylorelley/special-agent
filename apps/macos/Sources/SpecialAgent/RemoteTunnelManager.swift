...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.special-agent", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous SpecialAgent run already has an SSH listener on the expected port (common after restarts),
