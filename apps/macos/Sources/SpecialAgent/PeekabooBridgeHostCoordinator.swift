...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.special-agent", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: SpecialAgentPeekabooBridgeServices?
    private static var specialAgentSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("SpecialAgent", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = SpecialAgentPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.specialAgentSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.specialAgentSocketPath, privacy: .public)")
    }
...
@MainActor
private final class SpecialAgentPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.special-agent.peekaboo")
