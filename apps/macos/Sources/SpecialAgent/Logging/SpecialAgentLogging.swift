...
enum SpecialAgentLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = SpecialAgentOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = SpecialAgentFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        SpecialAgentLogging.bootstrapIfNeeded()
        let label = SpecialAgentLogging.makeLabel(subsystem: subsystem, category: category)
...
struct SpecialAgentOSLogHandler: LogHandler {
...
struct SpecialAgentFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = SpecialAgentLogging.parseLabel(self.label)
