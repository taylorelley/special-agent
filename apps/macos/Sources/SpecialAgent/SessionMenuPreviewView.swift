...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.special-agent", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> SpecialAgentSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: SpecialAgentSessionPreviewEntry,
    ...
    private static func cache(payload: SpecialAgentSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: SpecialAgentChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [SpecialAgentKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [SpecialAgentKit.AnyCodable]) -> [SpecialAgentChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(SpecialAgentChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: SpecialAgentChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: SpecialAgentChatMessage) -> Bool {
    ...
    private static func toolNames(for message: SpecialAgentChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: SpecialAgentChatMessage) -> String? {
