...
private let deepLinkLogger = Logger(subsystem: "ai.special-agent", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "SpecialAgent is paused", message: "Unpause SpecialAgent to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run SpecialAgent agent?", message: body) else { return }
        }
