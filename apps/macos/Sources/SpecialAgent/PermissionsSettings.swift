...
struct PermissionsSettings: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("Allow these so SpecialAgent can notify and capture when needed.")
                .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = SpecialAgentLocationMode.off.rawValue
    @AppStorage(locationPreciseKey) private var locationPreciseEnabled: Bool = true
    @State private var lastLocationModeRaw: String = SpecialAgentLocationMode.off.rawValue
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Picker("", selection: self.$locationModeRaw) {
                Text("Off").tag(SpecialAgentLocationMode.off.rawValue)
                Text("While Using").tag(SpecialAgentLocationMode.whileUsing.rawValue)
                Text("Always").tag(SpecialAgentLocationMode.always.rawValue)
            }
            .labelsHidden()
            .pickerStyle(.menu)
        ...
        .onChange(of: self.locationModeRaw) { _, newValue in
            ...
            guard let mode = SpecialAgentLocationMode(rawValue: newValue) else { return }
            Task {
    ...
    private var locationMode: SpecialAgentLocationMode {
        SpecialAgentLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: SpecialAgentLocationMode) async -> Bool {
