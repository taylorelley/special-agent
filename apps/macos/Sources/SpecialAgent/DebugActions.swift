...
enum DebugActions {
    private static let verboseDefaultsKey = "special-agent.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "special-agent.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = SpecialAgentPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "SpecialAgent", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = SpecialAgentPaths.configURL
        guard
