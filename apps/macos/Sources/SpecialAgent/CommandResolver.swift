...
enum CommandResolver {
    private static let projectRootDefaultsKey = "special-agent.gatewayProjectRootPath"
    private static let helperName = "special-agent"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let specialAgentEntry = root.appendingPathComponent("special-agent.mjs").path
        if FileManager().isReadableFile(atPath: specialAgentEntry) { return specialAgentEntry }
        let binEntry = root.appendingPathComponent("bin/special-agent.js").path
    ...
    static func errorCommand(with message: String) -> [String] {
        let script = """
        cat <<'__SPECIAL_AGENT_ERR__' >&2
        \(message)
        __SPECIAL_AGENT_ERR__
        exit 1
        """
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/special-agent")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let specialAgentPaths = self.specialAgentManagedPaths(home: home)
        if !specialAgentPaths.isEmpty {
            extras.insert(contentsOf: specialAgentPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + specialAgentPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func specialAgentManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".special-agent"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func specialAgentExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectSpecialAgentExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("special-agent.mjs").path,
            root.appendingPathComponent("bin/special-agent.js").path,
        ]
    ...
    static func hasAnySpecialAgentInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.specialAgentExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func specialAgentNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let specialAgentPath = self.projectSpecialAgentExecutable(projectRoot: root) {
                return [specialAgentPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "special-agent", subcommand] + extraArgs
            }
            if let specialAgentPath = self.specialAgentExecutable(searchPaths: searchPaths) {
                return [specialAgentPath, subcommand] + extraArgs
            }

            let missingEntry = """
            special-agent entrypoint missing (looked for dist/index.js or special-agent.mjs); run pnpm build.
            """
    ...
    static func specialAgentCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real special-agent CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/special-agent"
        ...
        \(cliSection)
        \(projectSection)
        if command -v special-agent >/dev/null 2>&1; then
          CLI="$(command -v special-agent)"
          special-agent \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/special-agent.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/special-agent.mjs"
            node "$PRJ/special-agent.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/special-agent.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/special-agent.js"
            node "$PRJ/bin/special-agent.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent special-agent"
          pnpm --silent special-agent \(quotedArgs);
        else
          echo "special-agent CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? SpecialAgentConfigFile.loadDict()
