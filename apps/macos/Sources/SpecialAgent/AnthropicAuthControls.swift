...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: SpecialAgentOAuthStore.AnthropicOAuthStatus = SpecialAgentOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: SpecialAgentOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = SpecialAgentOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = SpecialAgentOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try SpecialAgentOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. SpecialAgent can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: SpecialAgentOAuthStore.AnthropicOAuthStatus,
