...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: SpecialAgentProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.special-agent", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: SpecialAgentKit.AnyCodable]()) {
                $0[$1.key] = SpecialAgentKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: SpecialAgentProtocol.AnyCodable?) -> [String: SpecialAgentProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: SpecialAgentProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: SpecialAgentKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: SpecialAgentProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("special-agent.control.heartbeat")
    static let controlAgentEvent = Notification.Name("special-agent.control.agent")
}
