...
actor TalkModeRuntime {
    ...
    private let logger = Logger(subsystem: "ai.special-agent", category: "talk.runtime")
    private let ttsLogger = Logger(subsystem: "ai.special-agent", category: "talk.tts")
    ...
    private func latestAssistantText(sessionKey: String, since: Double? = nil) async -> String? {
        do {
            ...
            let decoded: [SpecialAgentChatMessage] = messages.compactMap { item in
