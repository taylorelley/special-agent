...
private let webChatSwiftLogger = Logger(subsystem: "ai.special-agent", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: SpecialAgentChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> SpecialAgentChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(SpecialAgentChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> SpecialAgentChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                SpecialAgentGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    SpecialAgentChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    SpecialAgentAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<SpecialAgentChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any SpecialAgentChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = SpecialAgentChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: SpecialAgentChatView(
            viewModel: vm,
        ...
        contentViewController: NSViewController) -> NSWindow
    {
        ...
        case .window:
            let window = NSWindow(
                ...
                backing: .buffered,
                defer: false)
            window.title = "SpecialAgent Chat"
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<SpecialAgentChatView>) -> NSViewController
    {
