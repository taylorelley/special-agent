...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (SpecialAgent token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: SpecialAgentOAuthStore.AnthropicOAuthStatus = SpecialAgentOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.special-agent", category: "anthropic-oauth")
...
enum SpecialAgentOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let specialAgentOAuthDirEnv = "SPECIAL_AGENT_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "SpecialAgent OAuth token file not found"
            case .unreadableFile: "SpecialAgent OAuth token file not readable"
            case .invalidJSON: "SpecialAgent OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in SpecialAgent OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "SpecialAgent OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.specialAgentOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".special-agent", isDirectory: true)
