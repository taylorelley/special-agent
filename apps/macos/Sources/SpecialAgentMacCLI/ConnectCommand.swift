...
struct ConnectOptions {
    ...
    var clientId: String = "special-agent-macos"
...
func runConnect(_ args: [String]) async {
    ...
    if opts.help {
        print("""
        special-agent-mac connect

        Usage:
          special-agent-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
        ...
        Options:
          ...
          --json             Emit JSON
          --client-id <id>   Override client id (default: special-agent-macos)
    ...
    do {
        ...
        let displayName = opts.displayName ?? Host.current().localizedName ?? "SpecialAgent macOS Debug CLI"
...
private func printConnectOutput(_ output: ConnectOutput, json: Bool) {
    ...
    print("SpecialAgent macOS Gateway Connect")
