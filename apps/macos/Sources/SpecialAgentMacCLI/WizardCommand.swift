...
func runWizardCommand(_ args: [String]) async {
    ...
    if opts.help {
        print("""
        special-agent-mac wizard

        Usage:
          special-agent-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
...
actor GatewayWizardClient {
    ...
    private func sendConnect() async throws {
        ...
        let clientId = "special-agent-macos"
        let clientMode = "ui"
        let role = "operator"
        ...
        let client: [String: ProtoAnyCodable] = [
            ...
            "displayName": ProtoAnyCodable(Host.current().localizedName ?? "SpecialAgent macOS Wizard CLI"),
