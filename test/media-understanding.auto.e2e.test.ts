...
import type { SpecialAgentConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("special-agent-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("special-agent-bin-sherpa-");
      const modelDir = await makeTempDir("special-agent-sherpa-model-");
      ...
      const cfg: SpecialAgentConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("special-agent-bin-whispercpp-");
      const modelDir = await makeTempDir("special-agent-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("special-agent-bin-gemini-");
      ...
      const cfg: SpecialAgentConfig = { tools: { media: { image: {} } } };
