...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("SPECIAL_AGENT_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.SPECIAL_AGENT_STATE_DIR = path.join(base, ".special-agent");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "special-agent-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".special-agent", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
