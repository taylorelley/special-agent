import type { SpecialAgentPluginApi, SpecialAgentPluginService } from "special-agent/plugin-sdk";
...
function normalizeDenyList(cfg: SpecialAgentPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: SpecialAgentPluginApi["config"]): string[] {
...
export default function register(api: SpecialAgentPluginApi) {
  ...
  const timerService: SpecialAgentPluginService = {
