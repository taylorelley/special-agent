...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.SPECIAL_AGENT_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "special-agent-nostr-"));
  process.env.SPECIAL_AGENT_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.SPECIAL_AGENT_STATE_DIR?.trim() || env.SPECIAL_AGENT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".special-agent");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.SPECIAL_AGENT_STATE_DIR = previous;
    }
