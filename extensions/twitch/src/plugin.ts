/**
 * Twitch channel plugin for SpecialAgent.
 *
...
import { buildChannelConfigSchema } from "special-agent/plugin-sdk";
import type {
 ...
 * for SpecialAgent. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: SpecialAgentConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: SpecialAgentConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: SpecialAgentConfig): boolean => {
