...
import type { ReplyPayload, SpecialAgentConfig } from "special-agent/plugin-sdk";
import { createReplyPrefixOptions } from "special-agent/plugin-sdk";
...
export type TwitchMonitorOptions = {
  ...
  accountId: string;
  config: unknown; // SpecialAgentConfig
...
async function processTwitchMessage(params: {
  ...
  const cfg = config as SpecialAgentConfig;
