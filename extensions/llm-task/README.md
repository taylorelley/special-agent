...
`special-agent.invoke --each`) without adding new Special Agent code per workflow.
...
This extension depends on Special Agent internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Special Agent extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.special-agent/extensions` as a standalone plugin directory.
