...
import { sleep } from "special-agent/plugin-sdk";
...
function resolveDefaultStorePath(config: VoiceCallConfig): string {
  const preferred = path.join(os.homedir(), ".special-agent", "voice-calls");
...
export function registerVoiceCallCli(params: {
  ...
  const root = program
    ...
    .addHelpText("after", () => `\nDocs: https://docs.special-agent.ai/cli/voicecall\n`);
