...
function resolveSpecialAgentRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.SPECIAL_AGENT_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["special-agent"]) {
  ...
  throw new Error("Unable to resolve core root. Set SPECIAL_AGENT_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveSpecialAgentRoot(), "dist", "extensionAPI.js");
