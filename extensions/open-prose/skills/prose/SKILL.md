...
metadata: { "special-agent": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Special Agent `sessions_spawn`
- **File I/O** == Special Agent `read`/`write`
- **Remote fetch** == Special Agent `web_fetch` (or `exec` with curl when POST is required)
