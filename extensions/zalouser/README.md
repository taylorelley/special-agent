...
Special Agent extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
special-agent channels login --channel zalouser
...
```bash
special-agent message send --channel zalouser --target <threadId> --message "Hello from Special Agent!"
```
...
```bash
special-agent channels login --channel zalouser              # Login via QR
special-agent channels login --channel zalouser --account work
special-agent channels status --probe
special-agent channels logout --channel zalouser
```
...
```bash
# Text
special-agent message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
special-agent message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `special-agent channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
special-agent channels login --channel zalouser --account work
special-agent message send --channel zalouser --account work --target <id> --message "Hello"
