...
- `fix: <summary> (special-agent#<PR>) thanks @<author>`

```sh
commit_msg="fix: <summary> (special-agent#$pr_number) thanks @$contrib"
...
echo "$subject" | rg -q "special-agent#$pr_number" || { echo "ERROR: commit subject missing special-agent#$pr_number"; exit 1; }
