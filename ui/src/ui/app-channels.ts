...
export async function handleWhatsAppStart(host: SpecialAgentApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: SpecialAgentApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: SpecialAgentApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: SpecialAgentApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: SpecialAgentApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: SpecialAgentApp): string {
...
export function handleNostrProfileCancel(host: SpecialAgentApp) {
...
export function handleNostrProfileToggleAdvanced(host: SpecialAgentApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: SpecialAgentApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: SpecialAgentApp) {
  const state = host.nostrProfileFormState;
