---
description: Update Special Agent from upstream when branch has diverged (ahead/behind)
---

# Special Agent Upstream Sync Workflow
...
```bash
# Kill running app
pkill -x "Special Agent" || true

# Move old version
mv /Applications/SpecialAgent.app /tmp/Special Agent-backup.app

# Install new build
cp -R dist/SpecialAgent.app /Applications/

# Launch
open /Applications/SpecialAgent.app
```
...
pnpm special-agent health

# Verify no zombie processes
ps aux | grep -E "(special-agent|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm special-agent agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update special-agent.json with fallback chains
...
pnpm special-agent agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
