...
```bash
SOCKET_DIR="${SPECIAL_AGENT_TMUX_SOCKET_DIR:-${SPECIAL_AGENT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/special-agent-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/special-agent.sock"
SESSION=special-agent-python
...
- Use `SPECIAL_AGENT_TMUX_SOCKET_DIR` (legacy `SPECIAL_AGENT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$SPECIAL_AGENT_TMUX_SOCKET_DIR/special-agent.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `SPECIAL_AGENT_TMUX_SOCKET_DIR`).
