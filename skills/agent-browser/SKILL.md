---
name: agent-browser
description: A fast headless browser automation CLI that enables agents to navigate, click, type, and snapshot web pages via structured commands.
          ...
          [
            {
              ...
              "bins": ["agent-browser"],
              "label": "Install agent-browser (node)",
            },
          ],
      },
  }
...
After installing the `agent-browser` npm package, run the browser install command to download Playwright browser binaries:
...
- Refs (`@e1`, `@e2`, etc.) are stable per page load but change on navigation.
