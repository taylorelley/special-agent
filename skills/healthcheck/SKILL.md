---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Special Agent deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Special Agent cron scheduling for periodic checks, or version status checks on a machine running Special Agent (laptop, workstation, Pi, VPS).
---

# Special Agent Host Hardening

## Overview

Assess and harden the host running Special Agent, then align it to a user-defined risk tolerance without breaking access. Use Special Agent security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Special Agent changes the host firewall, SSH, or OS updates; it does not.
...
5. Special Agent gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Special Agent can access sensitive data.
...
### 2) Run Special Agent security audits (read-only)

As part of the default read-only checks, run `special-agent security audit --deep`. Only offer alternatives if the user requests them:

1. `special-agent security audit` (faster, non-probing)
2. `special-agent security audit --json` (structured output)

Offer to apply Special Agent safe defaults (numbered):

1. `special-agent security audit --fix`

Be explicit that `--fix` only tightens Special Agent defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Special Agent version/update status (read-only)

As part of the default read-only checks, run `special-agent update status`.
...
- Credential hygiene notes (location of Special Agent creds, prefer disk encryption)
...
- Special Agent security audit (re-run)
...
After Special Agent install or first hardening pass, run at least one baseline audit and version check:

- `special-agent security audit`
- `special-agent security audit --deep`
- `special-agent update status`

Ongoing monitoring is recommended. Use the Special Agent cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `special-agent cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `special-agent update status`
...
Before creating, `special-agent cron list` and match on exact `name`. If found, `special-agent cron edit <id> ...`.
If not found, `special-agent cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `special-agent update status` (preferred for source checkouts and channels)
2. `npm view special-agent version` (published npm version)

## Special Agent command accuracy

Use only supported commands and flags:

- `special-agent security audit [--deep] [--fix] [--json]`
- `special-agent status` / `special-agent status --deep`
- `special-agent health --json`
- `special-agent update status`
- `special-agent cron add|list|runs|run`

Do not invent CLI flags or imply Special Agent enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Special Agent compaction:
